# 13.06 Sampling capacitor

## Scope

This is the canonical note for **Sampling capacitor** within **ADC and Analog Input**. The explanation belongs here; other notes should link to this topic rather than duplicate it.

## Core model

ADC performance depends on reference quality, source impedance, sampling time, quantization, noise and calibration. A high nominal resolution does not guarantee high effective resolution.

## Canonical mental model

```text
analog signal -> sample -> quantize -> digital code -> filter/calibrate
```
...
Be able to explain the requirement, architecture, mechanism, worst-case resource behavior, failure modes, observability and verification evidence for **Sampling capacitor** without relying on vendor-specific folklore.
