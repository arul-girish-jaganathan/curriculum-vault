# 28.02 Exception versus interrupt

## Scope

This is the canonical note for **Exception versus interrupt** within **Interrupts and Exception Fundamentals**. The explanation belongs here; other notes should link to this topic rather than duplicate it.

## Core model

Interrupts are asynchronous control transfers into firmware. Correctness depends on vector selection, masking, priority, handler behavior and shared-state rules.

## Canonical mental model

```text
event -> pending -> priority arbitration -> handler -> return
```
...
Be able to explain the requirement, architecture, mechanism, worst-case resource behavior, failure modes, observability and verification evidence for **Exception versus interrupt** without relying on vendor-specific folklore.
