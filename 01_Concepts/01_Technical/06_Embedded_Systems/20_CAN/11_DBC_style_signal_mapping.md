# 20.11 DBC-style signal mapping

## Scope

This is the canonical note for **DBC-style signal mapping** within **CAN and CAN-FD**. The explanation belongs here; other notes should link to this topic rather than duplicate it.

## Core model

CAN uses identifier-based arbitration so higher-priority messages win access to the bus. Error confinement and bus-off behavior are central to system reliability.

## Canonical mental model

```text
higher priority ID wins arbitration without corrupting the frame
```
...
Be able to explain the requirement, architecture, mechanism, worst-case resource behavior, failure modes, observability and verification evidence for **DBC-style signal mapping** without relying on vendor-specific folklore.
