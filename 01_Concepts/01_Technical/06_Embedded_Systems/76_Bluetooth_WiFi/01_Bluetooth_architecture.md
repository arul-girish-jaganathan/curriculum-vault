# 76.01 Bluetooth architecture

## Scope

This is the canonical note for **Bluetooth architecture** within **Bluetooth, Wi-Fi and Connectivity**. The explanation belongs here; other notes should link to this topic rather than duplicate it.

## Core model

Bluetooth and Wi-Fi expose different discovery, association, security, power and throughput trade-offs. Connectivity code benefits from an explicit state machine.

## Canonical mental model

```text
scan -> pair/associate -> configure -> connect -> recover
```
...
Be able to explain the requirement, architecture, mechanism, worst-case resource behavior, failure modes, observability and verification evidence for **Bluetooth architecture** without relying on vendor-specific folklore.
