# 35.01 Why use an RTOS

## Scope

This is the canonical note for **Why use an RTOS** within **RTOS Fundamentals**. The explanation belongs here; other notes should link to this topic rather than duplicate it.

## Core model

An RTOS provides scheduling, synchronization, timing and task-management services. It does not automatically make a design real-time; deadlines still need analysis.

## Canonical mental model

```text
tasks + scheduler + IPC + synchronization + timers
```
...
Be able to explain the requirement, architecture, mechanism, worst-case resource behavior, failure modes, observability and verification evidence for **Why use an RTOS** without relying on vendor-specific folklore.
