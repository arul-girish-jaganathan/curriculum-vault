# 52.10 Persistent boot metadata

## Scope

This is the canonical note for **Persistent boot metadata** within **Rollback, A/B and Recovery Strategies**. The explanation belongs here; other notes should link to this topic rather than duplicate it.

## Core model

Rollback depends on persistent boot metadata and a reliable notion of a confirmed-good image. The recovery path must itself remain bootable.

## Canonical mental model

```text
A active | B candidate
B boots -> health check -> confirm else revert A
```
...
Be able to explain the requirement, architecture, mechanism, worst-case resource behavior, failure modes, observability and verification evidence for **Persistent boot metadata** without relying on vendor-specific folklore.
