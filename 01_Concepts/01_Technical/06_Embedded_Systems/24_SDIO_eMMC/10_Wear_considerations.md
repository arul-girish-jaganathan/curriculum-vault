# 24.10 Wear considerations

## Scope

This is the canonical note for **Wear considerations** within **SDIO, eMMC and Removable Storage Interfaces**. The explanation belongs here; other notes should link to this topic rather than duplicate it.

## Core model

SD/eMMC interfaces combine protocol state, storage semantics and controller DMA. Power-loss behavior and write endurance must be considered at the storage-system level.

## Canonical mental model

```text
host controller -> DMA -> block storage -> filesystem
```
...
Be able to explain the requirement, architecture, mechanism, worst-case resource behavior, failure modes, observability and verification evidence for **Wear considerations** without relying on vendor-specific folklore.
