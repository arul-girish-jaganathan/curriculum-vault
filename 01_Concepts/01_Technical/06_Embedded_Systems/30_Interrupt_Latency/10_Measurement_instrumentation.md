# 30.10 Measurement instrumentation

## Scope

This is the canonical note for **Measurement instrumentation** within **Interrupt Latency and Determinism**. The explanation belongs here; other notes should link to this topic rather than duplicate it.

## Core model

Interrupt latency is the interval between an event becoming serviceable and useful handler work occurring. Measure it under the worst relevant cache, bus and masking conditions.

## Canonical mental model

```text
event_time -> entry -> ISR -> deferred work -> response
```
...
Be able to explain the requirement, architecture, mechanism, worst-case resource behavior, failure modes, observability and verification evidence for **Measurement instrumentation** without relying on vendor-specific folklore.
