# 34.12 Prevention strategy

## Scope

This is the canonical note for **Prevention strategy** within **Race Conditions and Deadlocks**. The explanation belongs here; other notes should link to this topic rather than duplicate it.

## Core model

Race conditions happen when correctness depends on an uncontrolled ordering of events. Deadlock additionally requires cyclic waiting for resources or synchronization conditions.

## Canonical mental model

```text
check(state) ----> use(state)
       ^ another actor changes state here
```
...
Be able to explain the requirement, architecture, mechanism, worst-case resource behavior, failure modes, observability and verification evidence for **Prevention strategy** without relying on vendor-specific folklore.
