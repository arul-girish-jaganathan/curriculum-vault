# 63.02 Hot path identification

## Scope

This is the canonical note for **Hot path identification** within **Embedded Performance Engineering**. The explanation belongs here; other notes should link to this topic rather than duplicate it.

## Core model

Performance engineering starts from a product metric—latency, throughput, CPU load, memory footprint or power—not from an optimization technique.

## Canonical mental model

```text
measure -> find hotspot -> change -> remeasure -> regression test
```
...
Be able to explain the requirement, architecture, mechanism, worst-case resource behavior, failure modes, observability and verification evidence for **Hot path identification** without relying on vendor-specific folklore.
