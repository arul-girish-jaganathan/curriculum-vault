# 33.10 Bootloader/application split

## Scope

This is the canonical note for **Bootloader/application split** within **Atomicity, Critical Sections and Synchronization**. The explanation belongs here; other notes should link to this topic rather than duplicate it.

## Core model

The linker and startup sequence turn a logical firmware image into actual target addresses. The map file is therefore an architecture artifact, not only a build by-product.

## Canonical mental model

```text
FLASH 0x08000000: vector/.text/.rodata
RAM   0x20000000: data/bss/stack
```
...
Be able to explain the requirement, architecture, mechanism, worst-case resource behavior, failure modes, observability and verification evidence for **Bootloader/application split** without relying on vendor-specific folklore.
