# 16.06 Coherent versus non-coherent memory

## Scope

This is the canonical note for **Coherent versus non-coherent memory** within **Advanced DMA Design**. The explanation belongs here; other notes should link to this topic rather than duplicate it.

## Core model

Advanced DMA pipelines can reach high throughput through descriptor chains and double buffering, but the state machine becomes distributed between CPU, DMA engine and peripheral.

## Canonical mental model

```text
RX DMA ring -> descriptor ownership -> CPU consume -> recycle
```
...
Be able to explain the requirement, architecture, mechanism, worst-case resource behavior, failure modes, observability and verification evidence for **Coherent versus non-coherent memory** without relying on vendor-specific folklore.
