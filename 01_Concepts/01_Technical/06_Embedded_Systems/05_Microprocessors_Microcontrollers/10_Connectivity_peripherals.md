# 05.10 Connectivity peripherals

## Scope

This is the canonical note for **Connectivity peripherals** within **Microprocessors and Microcontrollers**. The explanation belongs here; other notes should link to this topic rather than duplicate it.

## Core model

A microcontroller integrates a processing core with memory and peripherals. Selection depends on compute load, memory, timers, ADC, connectivity, power, package and lifecycle—not CPU frequency alone.

## Canonical mental model

```text
CPU + Flash + SRAM + GPIO + Timers + ADC + DMA + Comms
```
...
Be able to explain the requirement, architecture, mechanism, worst-case resource behavior, failure modes, observability and verification evidence for **Connectivity peripherals** without relying on vendor-specific folklore.
