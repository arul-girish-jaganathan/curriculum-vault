# 11.09 PWM timer relationship

## Scope

This is the canonical note for **PWM timer relationship** within **Timers, Counters and Capture/Compare**. The explanation belongs here; other notes should link to this topic rather than duplicate it.

## Core model

Timers convert a clock into precise state changes, timestamps, captures and periodic events. Prescaler, counter width and synchronization determine achievable resolution and range.

## Canonical mental model

```text
clock -> prescaler -> counter -> compare/capture -> event
```
...
Be able to explain the requirement, architecture, mechanism, worst-case resource behavior, failure modes, observability and verification evidence for **PWM timer relationship** without relying on vendor-specific folklore.
