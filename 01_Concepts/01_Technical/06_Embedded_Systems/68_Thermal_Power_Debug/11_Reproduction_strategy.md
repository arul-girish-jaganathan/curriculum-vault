# 68.11 Reproduction strategy

## Scope

This is the canonical note for **Reproduction strategy** within **Power/Thermal Debugging and Measurement**. The explanation belongs here; other notes should link to this topic rather than duplicate it.

## Core model

Embedded debugging combines software evidence with hardware state. The first incorrect state is usually more valuable than the final crash location.

## Canonical mental model

```text
reproduce -> capture first divergence -> isolate -> prove root cause
```
...
Be able to explain the requirement, architecture, mechanism, worst-case resource behavior, failure modes, observability and verification evidence for **Reproduction strategy** without relying on vendor-specific folklore.
