# 38.11 Synchronization invariants

## Scope

This is the canonical note for **Synchronization invariants** within **RTOS Synchronization Primitives**. The explanation belongs here; other notes should link to this topic rather than duplicate it.

## Core model

Atomicity prevents an operation from being observed half-complete, but atomicity alone does not define visibility, ordering or a correct multi-variable protocol.

## Canonical mental model

```text
read -> modify -> write
must be one protected state transition
```
...
Be able to explain the requirement, architecture, mechanism, worst-case resource behavior, failure modes, observability and verification evidence for **Synchronization invariants** without relying on vendor-specific folklore.
