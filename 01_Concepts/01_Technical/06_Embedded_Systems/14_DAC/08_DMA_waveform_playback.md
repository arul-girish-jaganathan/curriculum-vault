# 14.08 DMA waveform playback

## Scope

This is the canonical note for **DMA waveform playback** within **DAC and Analog Output**. The explanation belongs here; other notes should link to this topic rather than duplicate it.

## Core model

DAC output quality depends on resolution, settling, reference accuracy, output drive and filtering. Waveform generation often benefits from DMA-driven samples.

## Canonical mental model

```text
digital code -> DAC -> analog output -> filter/load
```
...
Be able to explain the requirement, architecture, mechanism, worst-case resource behavior, failure modes, observability and verification evidence for **DMA waveform playback** without relying on vendor-specific folklore.
