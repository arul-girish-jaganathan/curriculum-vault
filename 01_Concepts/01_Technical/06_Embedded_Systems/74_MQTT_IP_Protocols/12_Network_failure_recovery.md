# 74.12 Network failure recovery

## Scope

This is the canonical note for **Network failure recovery** within **IP, TCP/UDP and MQTT-Class Protocols**. The explanation belongs here; other notes should link to this topic rather than duplicate it.

## Core model

Application protocols such as MQTT add messaging semantics over IP transport. QoS, reconnect policy and state persistence determine behavior during outages.

## Canonical mental model

```text
connect -> authenticate -> subscribe/publish -> keepalive -> reconnect
```
...
Be able to explain the requirement, architecture, mechanism, worst-case resource behavior, failure modes, observability and verification evidence for **Network failure recovery** without relying on vendor-specific folklore.
