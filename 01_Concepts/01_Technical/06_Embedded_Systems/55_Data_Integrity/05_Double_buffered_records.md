# 55.05 Double-buffered records

## Scope

This is the canonical note for **Double-buffered records** within **Data Integrity and Persistent State**. The explanation belongs here; other notes should link to this topic rather than duplicate it.

## Core model

Persistent data is reliable only when corruption can be detected and the previous valid state can be recovered. Versioning and commit markers are common building blocks.

## Canonical mental model

```text
header + version + length + payload + CRC + commit marker
```
...
Be able to explain the requirement, architecture, mechanism, worst-case resource behavior, failure modes, observability and verification evidence for **Double-buffered records** without relying on vendor-specific folklore.
