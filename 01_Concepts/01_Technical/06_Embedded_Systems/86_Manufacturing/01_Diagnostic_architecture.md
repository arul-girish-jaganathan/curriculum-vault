# 86.01 Diagnostic architecture

## Scope

This is the canonical note for **Diagnostic architecture** within **Manufacturing Test and Production Diagnostics**. The explanation belongs here; other notes should link to this topic rather than duplicate it.

## Core model

Diagnostics should convert low-level failures into product-level evidence: fault type, context, severity, reset reason and relevant state.

## Canonical mental model

```text
fault -> code + context + timestamp + reset/persist policy
```
...
Be able to explain the requirement, architecture, mechanism, worst-case resource behavior, failure modes, observability and verification evidence for **Diagnostic architecture** without relying on vendor-specific folklore.
