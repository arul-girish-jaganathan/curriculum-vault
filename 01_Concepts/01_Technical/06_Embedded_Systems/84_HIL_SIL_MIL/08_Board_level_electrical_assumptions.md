# 84.08 Board-level electrical assumptions

## Scope

This is the canonical note for **Board-level electrical assumptions** within **MIL, SIL, HIL and Hardware Validation**. The explanation belongs here; other notes should link to this topic rather than duplicate it.

## Core model

The hardware/software boundary is a contract: register semantics, timing, reset state, interrupts, DMA ownership, electrical assumptions and error behavior must be explicit.

## Canonical mental model

```c
volatile uint32_t *STATUS = (uint32_t *)0x40000000u;
```
...
Be able to explain the requirement, architecture, mechanism, worst-case resource behavior, failure modes, observability and verification evidence for **Board-level electrical assumptions** without relying on vendor-specific folklore.
