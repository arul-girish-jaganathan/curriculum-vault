# 61.05 Breakpoint resources

## Scope

This is the canonical note for **Breakpoint resources** within **JTAG, SWD and Debug Access**. The explanation belongs here; other notes should link to this topic rather than duplicate it.

## Core model

JTAG/SWD provide controlled access to target state for programming and debug. Production products often need authenticated or disabled debug access for security.

## Canonical mental model

```text
Probe -> debug port -> CPU halt/run -> registers/memory/flash
```
...
Be able to explain the requirement, architecture, mechanism, worst-case resource behavior, failure modes, observability and verification evidence for **Breakpoint resources** without relying on vendor-specific folklore.
