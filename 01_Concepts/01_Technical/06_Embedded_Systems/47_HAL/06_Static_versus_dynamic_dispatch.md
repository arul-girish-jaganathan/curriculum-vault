# 47.06 Static versus dynamic dispatch

## Scope

This is the canonical note for **Static versus dynamic dispatch** within **HAL, LL and Abstraction Layers**. The explanation belongs here; other notes should link to this topic rather than duplicate it.

## Core model

A hardware abstraction layer should hide unstable hardware details while preserving required timing and error semantics. An abstraction that hides too little is not a real boundary; one that hides too much can impose cost.
...
Be able to explain the requirement, architecture, mechanism, worst-case resource behavior, failure modes, observability and verification evidence for **Static versus dynamic dispatch** without relying on vendor-specific folklore.
