# 22.12 Embedded Ethernet debugging

## Scope

This is the canonical note for **Embedded Ethernet debugging** within **Embedded Ethernet**. The explanation belongs here; other notes should link to this topic rather than duplicate it.

## Core model

Embedded Ethernet spans MAC, PHY, DMA descriptors, packet buffers and a network stack. Link state and memory ownership often dominate driver debugging.

## Canonical mental model

```text
MAC DMA -> driver ring -> network stack -> socket/application
```
...
Be able to explain the requirement, architecture, mechanism, worst-case resource behavior, failure modes, observability and verification evidence for **Embedded Ethernet debugging** without relying on vendor-specific folklore.
