# 64.06 Load-store behavior

## Scope

This is the canonical note for **Load-store behavior** within **Worst-Case Execution and WCET**. The explanation belongs here; other notes should link to this topic rather than duplicate it.

## Core model

The processor executes instructions through an implementation-specific pipeline while exposing an architectural state. Performance analysis must distinguish instruction semantics from microarchitectural behavior.
...
Be able to explain the requirement, architecture, mechanism, worst-case resource behavior, failure modes, observability and verification evidence for **Load-store behavior** without relying on vendor-specific folklore.
