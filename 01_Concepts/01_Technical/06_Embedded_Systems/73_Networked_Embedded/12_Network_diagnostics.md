# 73.12 Network diagnostics

## Scope

This is the canonical note for **Network diagnostics** within **Networked Embedded Systems**. The explanation belongs here; other notes should link to this topic rather than duplicate it.

## Core model

Networked embedded systems must survive disconnection, packet loss, latency, reordering and address changes rather than assuming a continuously healthy network.

## Canonical mental model

```text
application -> socket -> TCP/UDP -> IP -> Ethernet/Wi-Fi
```
...
Be able to explain the requirement, architecture, mechanism, worst-case resource behavior, failure modes, observability and verification evidence for **Network diagnostics** without relying on vendor-specific folklore.
