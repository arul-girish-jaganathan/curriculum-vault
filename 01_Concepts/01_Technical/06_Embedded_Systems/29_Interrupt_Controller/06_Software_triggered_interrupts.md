# 29.06 Software-triggered interrupts

## Scope

This is the canonical note for **Software-triggered interrupts** within **Interrupt Controllers and Priorities**. The explanation belongs here; other notes should link to this topic rather than duplicate it.

## Core model

Interrupt controllers arbitrate among many sources. Priority configuration is meaningful only when the system's preemption and critical-section policy is understood.

## Canonical mental model

```text
IRQ source -> controller -> priority/preemption -> CPU context
```
...
Be able to explain the requirement, architecture, mechanism, worst-case resource behavior, failure modes, observability and verification evidence for **Software-triggered interrupts** without relying on vendor-specific folklore.
