# 78.12 Secure boot diagnostics

## Scope

This is the canonical note for **Secure boot diagnostics** within **Secure Boot, Keys and Provisioning**. The explanation belongs here; other notes should link to this topic rather than duplicate it.

## Core model

Secure boot establishes a chain of trust from an immutable or protected root to the running firmware. Provisioning and recovery must preserve that chain.

## Canonical mental model

```text
ROM key -> verify bootloader -> verify image -> execute trusted code
```
...
Be able to explain the requirement, architecture, mechanism, worst-case resource behavior, failure modes, observability and verification evidence for **Secure boot diagnostics** without relying on vendor-specific folklore.
