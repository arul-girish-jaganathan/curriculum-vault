# 80.10 Control-flow integrity

## Scope

This is the canonical note for **Control-flow integrity** within **Memory Safety and Security Hardening**. The explanation belongs here; other notes should link to this topic rather than duplicate it.

## Core model

Memory safety failures can become code execution or privilege violations. Isolation, bounds checks, safer ownership, compiler hardening and MPU/MMU mechanisms reduce risk.

## Canonical mental model

```text
input -> validate bounds -> access -> invariant -> fault containment
```
...
Be able to explain the requirement, architecture, mechanism, worst-case resource behavior, failure modes, observability and verification evidence for **Control-flow integrity** without relying on vendor-specific folklore.
