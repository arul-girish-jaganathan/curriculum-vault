# 67.12 Power-performance trade-off

## Scope

This is the canonical note for **Power-performance trade-off** within **Low-Power Modes and Wakeup**. The explanation belongs here; other notes should link to this topic rather than duplicate it.

## Core model

Power management spans clocks, voltage, memory retention, peripherals and workload scheduling. Optimize energy per useful operation, not only instantaneous current.

## Canonical mental model

```text
run -> idle -> sleep -> wake -> restore
```
...
Be able to explain the requirement, architecture, mechanism, worst-case resource behavior, failure modes, observability and verification evidence for **Power-performance trade-off** without relying on vendor-specific folklore.
