# 83.05 Boundary-value tests

## Scope

This is the canonical note for **Boundary-value tests** within **Embedded Testing and Verification**. The explanation belongs here; other notes should link to this topic rather than duplicate it.

## Core model

Embedded verification needs both software-level isolation and real-target validation. A host test cannot prove hardware timing, register behavior or EMI-sensitive integration.

## Canonical mental model

```text
unit -> component -> integration -> system -> target/HIL
```
...
Be able to explain the requirement, architecture, mechanism, worst-case resource behavior, failure modes, observability and verification evidence for **Boundary-value tests** without relying on vendor-specific folklore.
