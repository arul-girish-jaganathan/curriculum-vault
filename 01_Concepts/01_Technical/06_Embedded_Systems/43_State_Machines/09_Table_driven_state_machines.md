# 43.09 Table-driven state machines

## Scope

This is the canonical note for **Table-driven state machines** within **State Machines**. The explanation belongs here; other notes should link to this topic rather than duplicate it.

## Core model

State machines make control behavior explicit by separating state, events, guards, actions and transitions. They are particularly useful for protocols and fault recovery.

## Canonical mental model

```text
State + Event + Guard -> Action -> New State
```
...
Be able to explain the requirement, architecture, mechanism, worst-case resource behavior, failure modes, observability and verification evidence for **Table-driven state machines** without relying on vendor-specific folklore.
