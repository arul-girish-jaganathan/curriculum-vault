# 22.05 Architectural versus implementation features

## Core idea

ARM is an architecture specification rather than a particular CPU implementation. Cortex cores can share the same ISA while differing significantly in pipeline, cache and power behavior.

This note is the canonical home for **Architectural versus implementation features** inside **Normal, Device and Strongly Ordered Memory**. Other notes should link here instead of repeating this explanation.
...
ARM is an architecture specification rather than a particular CPU implementation. Cortex cores can share the same ISA while differing significantly in pipeline, cache and power behavior. The architectural contract should be stated before the implementation detail.
