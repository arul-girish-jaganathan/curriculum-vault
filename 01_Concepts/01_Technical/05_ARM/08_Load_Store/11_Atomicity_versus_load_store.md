# 08.11 Atomicity versus load/store

## Core idea

ARM follows a load/store architecture: memory is accessed explicitly while most computation occurs on registers.

This note is the canonical home for **Atomicity versus load/store** inside **Load/Store Architecture**. Other notes should link here instead of repeating this explanation.
...
ARM follows a load/store architecture: memory is accessed explicitly while most computation occurs on registers. The architectural contract should be stated before the implementation detail.
