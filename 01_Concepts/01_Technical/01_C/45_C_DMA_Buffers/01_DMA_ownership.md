...
**DMA ownership** is the engineering concept concerned with **CPU/device ownership transfer, producer-consumer protocols, completion semantics, and lifetime.**.
...
For **DMA ownership**, adapt the example so that the critical invariant is visible at the boundary. **Example investigation:** Give a descriptor to DMA only after the CPU finishes all writes and reclaim it only after completion.
...
**document the ownership state machine and assert illegal transitions.**

## Topic-specific failure modes

CPU and DMA accessing the same buffer simultaneously, premature reuse, and ambiguous ownership states.
...
Before approving code involving **DMA ownership**, verify:
...
A Staff engineer should be able to explain not only **what DMA ownership does**, but **why the design remains correct when the compiler optimizes it, the target behaves adversarially, and the system enters failure or recovery states**.
