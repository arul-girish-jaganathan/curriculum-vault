...
> Canonical C topic note — chapter 45. This note follows the same deep-reference pattern as `01_C/13_C_Pointers/05_void_pointers.md`: precise definition → mechanism and rules → embedded implications → edge cases → example → verification → Staff-level reasoning.

## Definition

**Cache maintenance** is the engineering concept concerned with **clean/invalidate operations, cache-line granularity, coherency domains, and ordering around DMA.**.
...
For **Cache maintenance**, adapt the example so that the critical invariant is visible at the boundary. **Example investigation:** Clean a transmit buffer before DMA reads it and invalidate a receive buffer before the CPU consumes new data where the architecture requires it.
...
**use architecture-specific cache APIs and verify ranges are aligned to cache-line requirements.**

## Topic-specific failure modes

cleaning the wrong range, invalidating dirty CPU data, and assuming volatile fixes cache incoherency.
...
Before approving code involving **Cache maintenance**, verify:
...
A Staff engineer should be able to explain not only **what Cache maintenance does**, but **why the design remains correct when the compiler optimizes it, the target behaves adversarially, and the system enters failure or recovery states**.
