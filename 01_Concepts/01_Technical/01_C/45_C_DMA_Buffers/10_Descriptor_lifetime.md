...
**Descriptor lifetime** is the engineering concept concerned with **storage duration, alignment, reuse, ownership, and when a descriptor may be recycled.**.
...
For **Descriptor lifetime**, adapt the example so that the critical invariant is visible at the boundary. **Example investigation:** Keep descriptors in stable DMA-visible storage until hardware has definitively stopped referencing them.
...
**instrument allocation/recycle generations and assert lifetime transitions.**

## Topic-specific failure modes

stack-allocated descriptors, premature free/reuse, and changing descriptor fields while DMA owns them.
...
Before approving code involving **Descriptor lifetime**, verify:
...
A Staff engineer should be able to explain not only **what Descriptor lifetime does**, but **why the design remains correct when the compiler optimizes it, the target behaves adversarially, and the system enters failure or recovery states**.
