...
**IOMMU-facing pointers** is the engineering concept concerned with **CPU virtual addresses versus device/I/O virtual addresses, translation, mapping lifetime, and address width.**.
...
For **IOMMU-facing pointers**, adapt the example so that the critical invariant is visible at the boundary. **Example investigation:** Translate a CPU buffer through the platform's DMA mapping API instead of placing a raw C pointer in a descriptor.
...
**validate mapping attributes, address width, and unmap lifetime.**

## Topic-specific failure modes

truncating 64-bit addresses, stale mappings, identity-map assumptions, and confusing C pointer validity with device accessibility.
...
Before approving code involving **IOMMU-facing pointers**, verify:
...
A Staff engineer should be able to explain not only **what IOMMU-facing pointers does**, but **why the design remains correct when the compiler optimizes it, the target behaves adversarially, and the system enters failure or recovery states**.
