...
**Zero-copy buffers** is the engineering concept concerned with **avoiding copies through shared buffers, ownership transfer, lifetime, alignment, and cache policy.**.
...
For **Zero-copy buffers**, adapt the example so that the critical invariant is visible at the boundary. **Example investigation:** Pass a received DMA buffer directly to the parser only while ownership is explicitly transferred.
...
**model buffer states and test premature release/reuse.**

## Topic-specific failure modes

use-after-recycle, parser retaining a buffer too long, cache incoherency, and mutation by two owners.
...
Before approving code involving **Zero-copy buffers**, verify:
...
A Staff engineer should be able to explain not only **what Zero-copy buffers does**, but **why the design remains correct when the compiler optimizes it, the target behaves adversarially, and the system enters failure or recovery states**.
