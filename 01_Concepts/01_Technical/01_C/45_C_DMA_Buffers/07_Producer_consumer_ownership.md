...
**Producer-consumer ownership** is the engineering concept concerned with **formal ownership state machines between CPU, DMA, ISR, and tasks.**.
...
For **Producer-consumer ownership**, adapt the example so that the critical invariant is visible at the boundary. **Example investigation:** Represent FREE → CPU_FILL → DEVICE_OWNED → CPU_PROCESS → FREE explicitly.
...
**review every transition and prove one owner at a time.**

## Topic-specific failure modes

implicit ownership encoded only by comments or unrelated flags and multiple producers without arbitration.
...
Before approving code involving **Producer-consumer ownership**, verify:
...
A Staff engineer should be able to explain not only **what Producer-consumer ownership does**, but **why the design remains correct when the compiler optimizes it, the target behaves adversarially, and the system enters failure or recovery states**.
