...
**DMA debugging** is the engineering concept concerned with **descriptor dumps, ownership tracing, cache diagnosis, bus faults, timing, and reproducible failure capture.**.
...
For **DMA debugging**, adapt the example so that the critical invariant is visible at the boundary. **Example investigation:** Log a descriptor index, owner state, device status, buffer address, and cache-operation boundaries when a transfer fails.
...
**capture non-halting traces and compare CPU-visible and device-visible state.**

## Topic-specific failure modes

debugger halts changing timing, reading MMIO destructively, and inspecting a CPU pointer that is not a valid device address.
...
Before approving code involving **DMA debugging**, verify:
...
A Staff engineer should be able to explain not only **what DMA debugging does**, but **why the design remains correct when the compiler optimizes it, the target behaves adversarially, and the system enters failure or recovery states**.
