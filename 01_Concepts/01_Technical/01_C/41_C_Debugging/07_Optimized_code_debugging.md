...
**Optimized-code debugging** is the engineering concept concerned with **as-if transformations, instruction scheduling, register allocation, inlining, dead-code elimination, LTO, and imperfect source mapping.**.
...
For **Optimized-code debugging**, adapt the example so that the critical invariant is visible at the boundary. **Example investigation:** Debug an `-O2` state machine by starting from the PC/disassembly and reconstructing the source-level state afterward.
...
**compare `-O0` and production-like builds, but prove the production failure on the production-like binary.**

## Topic-specific failure modes

single-stepping is not source execution; variables may disappear; line order may look strange; optimization can expose latent undefined behavior.
...
Before approving code involving **Optimized-code debugging**, verify:
...
A Staff engineer should be able to explain not only **what Optimized-code debugging does**, but **why the design remains correct when the compiler optimizes it, the target behaves adversarially, and the system enters failure or recovery states**.
