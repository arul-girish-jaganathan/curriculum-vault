...
> Canonical C topic note — chapter 41. This note follows the same deep-reference pattern as `01_C/13_C_Pointers/05_void_pointers.md`: precise definition → mechanism and rules → embedded implications → edge cases → example → verification → Staff-level reasoning.

## Definition

**Source-level debugging** is the engineering concept concerned with **DWARF/debug information, source-to-instruction mapping, variable locations, line tables, inlining, and the fact that debugging views are not ISO C semantics.**.
...
For **Source-level debugging**, adapt the example so that the critical invariant is visible at the boundary. **Example investigation:** Inspect an optimized checksum routine and compare the source variable view with disassembly when a variable is reported as optimized out.
...
**reproduce with the exact ELF, compiler flags and symbols; compare source view, disassembly, registers, and map file.**

## Topic-specific failure modes

source line does not imply one instruction; optimized variables may not exist in memory; stale ELF/symbol files invalidate conclusions.
...
Before approving code involving **Source-level debugging**, verify:
...
A Staff engineer should be able to explain not only **what Source-level debugging does**, but **why the design remains correct when the compiler optimizes it, the target behaves adversarially, and the system enters failure or recovery states**.
