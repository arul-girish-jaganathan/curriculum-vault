...
**Fault localization** is the engineering concept concerned with **binary search of the execution path, assertions, sentinels, instrumentation, boundary isolation, and evidence-driven narrowing.**.
...
For **Fault localization**, adapt the example so that the critical invariant is visible at the boundary. **Example investigation:** Split a long sensor pipeline into stages and add monotonic event markers until the first bad state is localized.
...
**define the invariant and identify the earliest transition that violates it.**

## Topic-specific failure modes

instrumentation changes timing, checks can mask memory corruption, and the first observed failure may be downstream.
...
Before approving code involving **Fault localization**, verify:
...
A Staff engineer should be able to explain not only **what Fault localization does**, but **why the design remains correct when the compiler optimizes it, the target behaves adversarially, and the system enters failure or recovery states**.
