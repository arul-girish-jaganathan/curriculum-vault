...
**Breakpoints** is the engineering concept concerned with **software versus hardware breakpoints, address comparators, flash/ROM restrictions, conditional breakpoints, and timing perturbation.**.
...
For **Breakpoints**, adapt the example so that the critical invariant is visible at the boundary. **Example investigation:** Stop at a defensive error path and inspect the input length, caller, ownership state, and fault context.
...
**verify the image/symbol match and replace halting with trace/GPIO/counters when timing is part of the bug.**

## Topic-specific failure modes

limited hardware resources, read-only code, inlined/eliminated code, and breakpoints that hide races or deadline failures.
...
Before approving code involving **Breakpoints**, verify:
...
A Staff engineer should be able to explain not only **what Breakpoints does**, but **why the design remains correct when the compiler optimizes it, the target behaves adversarially, and the system enters failure or recovery states**.
