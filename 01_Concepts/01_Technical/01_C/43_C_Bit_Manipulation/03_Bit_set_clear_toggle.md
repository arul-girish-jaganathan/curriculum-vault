...
**Bit set/clear/toggle** is the engineering concept concerned with **OR/AND-NOT/XOR semantics, atomicity concerns, and register-specific write semantics.**.
...
For **Bit set/clear/toggle**, adapt the example so that the critical invariant is visible at the boundary. **Example investigation:** Set one flag with OR and clear one flag with AND-NOT while preserving neighbors.
...
**inspect the generated access sequence and use atomic/set-clear hardware when required.**

## Topic-specific failure modes

read-modify-write races, write-one-to-clear behavior, and XOR toggles that are not idempotent.
...
Before approving code involving **Bit set/clear/toggle**, verify:
...
A Staff engineer should be able to explain not only **what Bit set/clear/toggle does**, but **why the design remains correct when the compiler optimizes it, the target behaves adversarially, and the system enters failure or recovery states**.
