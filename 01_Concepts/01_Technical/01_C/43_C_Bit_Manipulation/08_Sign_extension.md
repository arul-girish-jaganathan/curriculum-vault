...
**Sign extension** is the engineering concept concerned with **conversion from narrow signed representations to wider integers and explicit bit-pattern interpretation.**.
...
For **Sign extension**, adapt the example so that the critical invariant is visible at the boundary. **Example investigation:** Decode a signed 12-bit two's-complement field by sign-extending only after extracting its 12 bits.
...
**test positive maximum, negative minimum, and -1 encodings.**

## Topic-specific failure modes

shifting signed negatives, implementation assumptions, and confusing numeric conversion with raw bit reinterpretation.
...
Before approving code involving **Sign extension**, verify:
...
A Staff engineer should be able to explain not only **what Sign extension does**, but **why the design remains correct when the compiler optimizes it, the target behaves adversarially, and the system enters failure or recovery states**.
