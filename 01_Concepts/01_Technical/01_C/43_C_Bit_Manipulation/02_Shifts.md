...
**Shifts** is the engineering concept concerned with **integer promotions, valid shift counts, signed versus unsigned shifts, and width-dependent behavior.**.
...
For **Shifts**, adapt the example so that the critical invariant is visible at the boundary. **Example investigation:** Use `UINT32_C(1) << bit` for a 32-bit unsigned mask with a validated bit index.
...
**test count 0, width-1, and invalid counts at the API boundary.**

## Topic-specific failure modes

negative counts, counts >= width, left shift overflow on signed values, and assuming arithmetic right shift is universally portable.
...
Before approving code involving **Shifts**, verify:
...
A Staff engineer should be able to explain not only **what Shifts does**, but **why the design remains correct when the compiler optimizes it, the target behaves adversarially, and the system enters failure or recovery states**.
