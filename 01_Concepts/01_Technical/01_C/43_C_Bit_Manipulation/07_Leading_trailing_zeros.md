...
**Leading/trailing zeros** is the engineering concept concerned with **bit-scan primitives, zero-input preconditions, width semantics, and compiler builtins.**.
...
For **Leading/trailing zeros**, adapt the example so that the critical invariant is visible at the boundary. **Example investigation:** Use a guarded CLZ operation to find the highest active queue bit.
...
**test zero separately and verify the returned index convention.**

## Topic-specific failure modes

calling zero-undefined builtins on zero, wrong integer width, and off-by-one indexing.
...
Before approving code involving **Leading/trailing zeros**, verify:
...
A Staff engineer should be able to explain not only **what Leading/trailing zeros does**, but **why the design remains correct when the compiler optimizes it, the target behaves adversarially, and the system enters failure or recovery states**.
