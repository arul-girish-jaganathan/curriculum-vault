...
**Return-code conventions** is the engineering concept concerned with **integer/enumerated status contracts, success/failure polarity, ownership rules, and ABI-visible return values.**.
...
For **Return-code conventions**, adapt the example so that the critical invariant is visible at the boundary. **Example investigation:** Return a status while placing successful output in an out-parameter and document which outputs are valid on each status.
...
**review every caller and test each failure path, not only the happy path.**

## Topic-specific failure modes

mixing Boolean and error-code conventions, losing errors at call boundaries, and ambiguous zero/nonzero meanings.
...
Before approving code involving **Return-code conventions**, verify:
...
A Staff engineer should be able to explain not only **what Return-code conventions does**, but **why the design remains correct when the compiler optimizes it, the target behaves adversarially, and the system enters failure or recovery states**.
