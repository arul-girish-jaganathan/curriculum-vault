...
**Out-parameters** is the engineering concept concerned with **pointer-based output contracts, initialization, ownership, aliasing, const qualification, and failure atomicity.**.
...
For **Out-parameters**, adapt the example so that the critical invariant is visible at the boundary. **Example investigation:** Fill a caller-provided structure only after all validation succeeds, or explicitly document partial output.
...
**review pointer validity, alignment, lifetime, aliasing, and output-validity rules.**

## Topic-specific failure modes

writing through invalid pointers, aliasing input/output objects unexpectedly, and returning success with incomplete output.
...
Before approving code involving **Out-parameters**, verify:
...
A Staff engineer should be able to explain not only **what Out-parameters does**, but **why the design remains correct when the compiler optimizes it, the target behaves adversarially, and the system enters failure or recovery states**.
