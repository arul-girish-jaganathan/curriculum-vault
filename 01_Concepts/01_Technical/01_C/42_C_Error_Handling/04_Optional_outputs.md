...
**Optional outputs** is the engineering concept concerned with **nullable output pointers, feature-dependent results, partial outputs, and precise validity contracts.**.
...
For **Optional outputs**, adapt the example so that the critical invariant is visible at the boundary. **Example investigation:** Allow a caller to pass NULL for an optional diagnostic output while still guaranteeing the primary result contract.
...
**initialize outputs deliberately and test NULL, success, and every failure combination.**

## Topic-specific failure modes

dereferencing optional outputs, leaving stale output data after failure, and ambiguous partial validity.
...
Before approving code involving **Optional outputs**, verify:
...
A Staff engineer should be able to explain not only **what Optional outputs does**, but **why the design remains correct when the compiler optimizes it, the target behaves adversarially, and the system enters failure or recovery states**.
