...
**API failure contracts** is the engineering concept concerned with **preconditions, postconditions, output validity, ownership, retryability, timing, and documented failure taxonomy.**.
...
For **API failure contracts**, adapt the example so that the critical invariant is visible at the boundary. **Example investigation:** Specify exactly what a driver API guarantees on timeout, invalid input, busy state, and hardware fault.
...
**turn the contract into unit tests and caller-side review questions.**

## Topic-specific failure modes

contracts that describe only return values, unspecified output contents, and undocumented retry/ownership behavior.
...
Before approving code involving **API failure contracts**, verify:
...
A Staff engineer should be able to explain not only **what API failure contracts does**, but **why the design remains correct when the compiler optimizes it, the target behaves adversarially, and the system enters failure or recovery states**.
