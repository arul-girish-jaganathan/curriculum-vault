...
**Embedded degraded modes** is the engineering concept concerned with **safe fallback behavior, feature shedding, retry budgets, watchdog interaction, and deterministic reduced functionality.**.
...
For **Embedded degraded modes**, adapt the example so that the critical invariant is visible at the boundary. **Example investigation:** If a sensor becomes unavailable, freeze the last trusted value for a bounded interval and transition to a documented degraded mode.
...
**test entry, steady-state, recovery, and repeated-fault behavior.**

## Topic-specific failure modes

silent permanent degradation, retry loops, unsafe defaults, and recovery oscillation.
...
Before approving code involving **Embedded degraded modes**, verify:
...
A Staff engineer should be able to explain not only **what Embedded degraded modes does**, but **why the design remains correct when the compiler optimizes it, the target behaves adversarially, and the system enters failure or recovery states**.
