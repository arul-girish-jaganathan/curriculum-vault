...
**Error propagation** is the engineering concept concerned with **preserving failure identity across layers, mapping errors between domains, and preventing accidental swallowing.**.
...
For **Error propagation**, adapt the example so that the critical invariant is visible at the boundary. **Example investigation:** Map a low-level timeout into a subsystem status while retaining the original diagnostic code for telemetry.
...
**trace one injected failure through every layer and verify the final contract.**

## Topic-specific failure modes

overly broad error mapping, ignoring rare errors, retrying non-retryable failures, and destroying causal context.
...
Before approving code involving **Error propagation**, verify:
...
A Staff engineer should be able to explain not only **what Error propagation does**, but **why the design remains correct when the compiler optimizes it, the target behaves adversarially, and the system enters failure or recovery states**.
