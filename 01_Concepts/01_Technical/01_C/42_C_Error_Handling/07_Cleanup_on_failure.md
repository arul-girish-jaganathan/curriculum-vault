...
**Cleanup on failure** is the engineering concept concerned with **single-exit cleanup, staged acquisition, ownership transfer, and idempotent release.**.
...
For **Cleanup on failure**, adapt the example so that the critical invariant is visible at the boundary. **Example investigation:** Acquire peripheral resources in stages and unwind them in reverse order when initialization fails.
...
**test every failure injection point and prove resource counts return to baseline.**

## Topic-specific failure modes

double release, release of uninitialized resources, cleanup paths that themselves fail, and hidden side effects.
...
Before approving code involving **Cleanup on failure**, verify:
...
A Staff engineer should be able to explain not only **what Cleanup on failure does**, but **why the design remains correct when the compiler optimizes it, the target behaves adversarially, and the system enters failure or recovery states**.
