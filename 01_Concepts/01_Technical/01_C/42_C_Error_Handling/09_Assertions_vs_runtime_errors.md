...
**Assertions vs runtime errors** is the engineering concept concerned with **developer invariants versus expected environmental/runtime failures, build policy, and recovery expectations.**.
...
For **Assertions vs runtime errors**, adapt the example so that the critical invariant is visible at the boundary. **Example investigation:** Assert an impossible internal state in a debug build while returning a recoverable timeout for a disconnected device.
...
**classify each condition explicitly as invariant violation, environmental failure, or input rejection.**

## Topic-specific failure modes

asserting on normal field conditions, compiling away checks that were actually safety requirements, and continuing after invariant corruption.
...
Before approving code involving **Assertions vs runtime errors**, verify:
...
A Staff engineer should be able to explain not only **what Assertions vs runtime errors does**, but **why the design remains correct when the compiler optimizes it, the target behaves adversarially, and the system enters failure or recovery states**.
