...
**errno semantics** is the engineering concept concerned with **the C library `errno` model, when a function may set it, when it is meaningful, and why it is not a universal embedded error mechanism.**.
...
For **errno semantics**, adapt the example so that the critical invariant is visible at the boundary. **Example investigation:** Capture `errno` immediately after a failing library call before another operation can overwrite the diagnostic state.
...
**test the actual libc contract and document which calls produce which errors.**

## Topic-specific failure modes

reading errno after a successful call, assuming numeric values are portable, and using thread-local/RTOS behavior without checking the implementation.
...
Before approving code involving **errno semantics**, verify:
...
A Staff engineer should be able to explain not only **what errno semantics does**, but **why the design remains correct when the compiler optimizes it, the target behaves adversarially, and the system enters failure or recovery states**.
