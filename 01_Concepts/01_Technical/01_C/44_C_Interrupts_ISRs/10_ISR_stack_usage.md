...
**ISR stack usage** is the engineering concept concerned with **exception stacking, compiler-generated frames, nested depth, local arrays, and stack guards.**.
...
For **ISR stack usage**, adapt the example so that the critical invariant is visible at the boundary. **Example investigation:** Estimate worst-case stack from architectural exception state plus compiler frame usage for the deepest nesting path.
...
**use map/disassembly/stack painting and worst-case nesting analysis.**

## Topic-specific failure modes

large local buffers, recursion, compiler-option changes, and ignoring nested interrupts.
...
Before approving code involving **ISR stack usage**, verify:
...
A Staff engineer should be able to explain not only **what ISR stack usage does**, but **why the design remains correct when the compiler optimizes it, the target behaves adversarially, and the system enters failure or recovery states**.
