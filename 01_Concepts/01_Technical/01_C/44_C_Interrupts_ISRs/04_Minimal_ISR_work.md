...
**Minimal ISR work** is the engineering concept concerned with **capture-now/process-later architecture, bounded critical work, and latency budgets.**.
...
For **Minimal ISR work**, adapt the example so that the critical invariant is visible at the boundary. **Example investigation:** Timestamp and copy only the data required to process a received byte later.
...
**measure entry-to-exit cycles and queue backlog under worst-case interrupt rate.**

## Topic-specific failure modes

doing parsing, logging, allocation, or long calculations inside the ISR.
...
Before approving code involving **Minimal ISR work**, verify:
...
A Staff engineer should be able to explain not only **what Minimal ISR work does**, but **why the design remains correct when the compiler optimizes it, the target behaves adversarially, and the system enters failure or recovery states**.
