...
**Reentrancy** is the engineering concept concerned with **nested calls, shared static state, callbacks, signal/ISR interruption, and thread-safe API design.**.
...
For **Reentrancy**, adapt the example so that the critical invariant is visible at the boundary. **Example investigation:** Refactor a static scratch buffer into caller-owned storage so an ISR cannot corrupt foreground state.
...
**force nested/interrupted execution in tests and inspect all shared state.**

## Topic-specific failure modes

hidden global state, non-reentrant library calls, and callbacks that reenter the same subsystem.
...
Before approving code involving **Reentrancy**, verify:
...
A Staff engineer should be able to explain not only **what Reentrancy does**, but **why the design remains correct when the compiler optimizes it, the target behaves adversarially, and the system enters failure or recovery states**.
