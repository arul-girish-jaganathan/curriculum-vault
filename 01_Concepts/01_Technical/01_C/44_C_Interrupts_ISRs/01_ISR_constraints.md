...
**ISR constraints** is the engineering concept concerned with **interrupt entry/exit ABI, bounded execution, register preservation, nesting, and forbidden/blocking operations.**.
...
For **ISR constraints**, adapt the example so that the critical invariant is visible at the boundary. **Example investigation:** Capture a hardware status word and enqueue a compact event before returning.
...
**measure worst-case latency and inspect the compiler/ABI interrupt wrapper.**

## Topic-specific failure modes

unbounded loops, blocking calls, dynamic allocation, heavy logging, and assumptions that ISR execution is ordinary C function execution.
...
Before approving code involving **ISR constraints**, verify:
...
A Staff engineer should be able to explain not only **what ISR constraints does**, but **why the design remains correct when the compiler optimizes it, the target behaves adversarially, and the system enters failure or recovery states**.
