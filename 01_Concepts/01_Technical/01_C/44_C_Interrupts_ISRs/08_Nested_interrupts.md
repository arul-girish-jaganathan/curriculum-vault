...
**Nested interrupts** is the engineering concept concerned with **priority levels, preemption, stacking, latency amplification, and shared-resource protection.**.
...
For **Nested interrupts**, adapt the example so that the critical invariant is visible at the boundary. **Example investigation:** Allow a high-priority fault ISR to preempt a lower-priority peripheral ISR while keeping both bounded.
...
**calculate maximum nesting depth and stack usage.**

## Topic-specific failure modes

unbounded nesting, priority inversion, and assuming interrupt masking protects all cores/resources.
...
Before approving code involving **Nested interrupts**, verify:
...
A Staff engineer should be able to explain not only **what Nested interrupts does**, but **why the design remains correct when the compiler optimizes it, the target behaves adversarially, and the system enters failure or recovery states**.
