...
**ISR review checklist** is the engineering concept concerned with **context legality, boundedness, memory access, synchronization, stack, latency, and fault behavior.**.
...
For **ISR review checklist**, adapt the example so that the critical invariant is visible at the boundary. **Example investigation:** Review an ISR by tracing every call and shared object it touches rather than reviewing only the ISR body.
...
**use a repeatable checklist and attach timing/stack evidence to the review.**

## Topic-specific failure modes

accepting 'small' source code without proving hidden call cost or synchronization correctness.
...
Before approving code involving **ISR review checklist**, verify:
...
A Staff engineer should be able to explain not only **what ISR review checklist does**, but **why the design remains correct when the compiler optimizes it, the target behaves adversarially, and the system enters failure or recovery states**.
