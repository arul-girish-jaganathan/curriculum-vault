...
**Interrupt-safe APIs** is the engineering concept concerned with **reentrancy, lockability, allocation, blocking, and API classification for ISR context.**.
...
For **Interrupt-safe APIs**, adapt the example so that the critical invariant is visible at the boundary. **Example investigation:** Provide an ISR-safe `try_enqueue_event()` that never blocks and has bounded execution.
...
**annotate APIs by context and enforce the rule in review/static analysis.**

## Topic-specific failure modes

calling mutex/stdio/heap APIs that can block or share non-reentrant state.
...
Before approving code involving **Interrupt-safe APIs**, verify:
...
A Staff engineer should be able to explain not only **what Interrupt-safe APIs does**, but **why the design remains correct when the compiler optimizes it, the target behaves adversarially, and the system enters failure or recovery states**.
