...
This is the canonical note for **Thread-local state** inside **Threads and Multithreading**. Other OS notes should link here rather than copy the explanation.
...
For **Thread-local state**, identify the exact state stored by the kernel, who can change it, what event causes progress, and what other subsystem must cooperate.
