# 07.07 Scheduler initialization

## Core idea

Boot is a staged transfer from firmware-controlled execution into a fully initialized OS. CPU state, memory, interrupts, device discovery, root storage and initial user space all have ordering dependencies.

This is the canonical note for **Scheduler initialization** inside **System Boot and OS Initialization**. Other OS notes should link here rather than copy the explanation.
...
For **Scheduler initialization**, identify the exact state stored by the kernel, who can change it, what event causes progress, and what other subsystem must cooperate.
