# 13.01 Program versus process

## Core idea

A process is a program in execution together with its address space, CPU context, resources and OS-management state. Modern OS documentation and GFG distinguish the passive executable from the active process. citeturn388804search6

This is the canonical note for **Program versus process** inside **Parent, Child and Orphan Processes**. Other OS notes should link here rather than copy the explanation.

## Mental model

```text
program -> loaded image -> process -> resources -> exit
```
...
For **Program versus process**, identify the exact state stored by the kernel, who can change it, what event causes progress, and what other subsystem must cooperate.
