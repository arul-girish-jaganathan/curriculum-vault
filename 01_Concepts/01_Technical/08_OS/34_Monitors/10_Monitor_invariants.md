# 34.10 Monitor invariants

## Core idea

A monitor combines shared state, mutual exclusion and condition synchronization. The predicate in a condition-variable wait is the real correctness condition.

This is the canonical note for **Monitor invariants** inside **Monitors and Condition Synchronization**. Other OS notes should link here rather than copy the explanation.
...
For **Monitor invariants**, identify the exact state stored by the kernel, who can change it, what event causes progress, and what other subsystem must cooperate.
