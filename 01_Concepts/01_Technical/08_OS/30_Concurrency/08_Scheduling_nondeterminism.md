# 30.08 Scheduling nondeterminism

## Core idea

Concurrency describes overlapping progress, while parallelism describes simultaneous execution. Shared mutable state requires synchronization or a deliberate ownership protocol.

This is the canonical note for **Scheduling nondeterminism** inside **Concurrency and Parallelism**. Other OS notes should link here rather than copy the explanation.
...
For **Scheduling nondeterminism**, identify the exact state stored by the kernel, who can change it, what event causes progress, and what other subsystem must cooperate.
