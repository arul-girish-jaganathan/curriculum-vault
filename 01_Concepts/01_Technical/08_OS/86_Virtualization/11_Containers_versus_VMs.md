# 86.11 Containers versus VMs

## Core idea

Hypervisors virtualize CPU, memory and devices so guests can run with controlled access to hardware; containers share the host kernel while isolating processes and resources.

This is the canonical note for **Containers versus VMs** inside **Virtualization and Hypervisors**. Other OS notes should link here rather than copy the explanation.

## Mental model

```text
guest -> virtual CPU/memory/device -> hypervisor -> hardware
```
...
For **Containers versus VMs**, identify the exact state stored by the kernel, who can change it, what event causes progress, and what other subsystem must cooperate.
