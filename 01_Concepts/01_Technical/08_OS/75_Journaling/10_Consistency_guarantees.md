# 75.10 Consistency guarantees

## Core idea

Journaling records intended metadata/data updates so a crash can be followed by deterministic replay or rollback instead of leaving arbitrary partial metadata state.

This is the canonical note for **Consistency guarantees** inside **Journaling and Crash Consistency**. Other OS notes should link here rather than copy the explanation.
...
For **Consistency guarantees**, identify the exact state stored by the kernel, who can change it, what event causes progress, and what other subsystem must cooperate.
