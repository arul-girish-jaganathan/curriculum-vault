# 85.10 Control-flow integrity

## Core idea

OS security layers authentication, authorization, isolation, memory protection, hardening, auditing and recovery around potentially hostile software.

This is the canonical note for **Control-flow integrity** inside **Operating System Security**. Other OS notes should link here rather than copy the explanation.

## Mental model

```text
identity -> authorization -> isolation -> audit -> response
```
...
For **Control-flow integrity**, identify the exact state stored by the kernel, who can change it, what event causes progress, and what other subsystem must cooperate.
