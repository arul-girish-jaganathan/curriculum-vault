# 88.12 Performance regression

## Core idea

OS performance requires observing CPU scheduling, memory faults, cache/TLB behavior, I/O wait, synchronization and workload distribution together.

This is the canonical note for **Performance regression** inside **OS Performance, Debugging and Observability**. Other OS notes should link here rather than copy the explanation.

## Mental model

```text
workload -> scheduler/memory/I/O -> observed latency/throughput
```
...
For **Performance regression**, identify the exact state stored by the kernel, who can change it, what event causes progress, and what other subsystem must cooperate.
