# 44.07 FIFO filesystem node

## Core idea

Pipes provide byte streams between processes; FIFOs extend the model to named filesystem endpoints.

This is the canonical note for **FIFO filesystem node** inside **Pipes and FIFOs**. Other OS notes should link here rather than copy the explanation.
...
For **FIFO filesystem node**, identify the exact state stored by the kernel, who can change it, what event causes progress, and what other subsystem must cooperate.
