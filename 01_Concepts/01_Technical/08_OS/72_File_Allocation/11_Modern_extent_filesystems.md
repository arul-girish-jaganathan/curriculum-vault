# 72.11 Modern extent filesystems

## Core idea

Contiguous allocation simplifies access but is vulnerable to external fragmentation; linked and indexed schemes trade random-access speed and metadata overhead differently. Modern extent allocation combines contiguous runs with flexible growth.

This is the canonical note for **Modern extent filesystems** inside **File Allocation Methods**. Other OS notes should link here rather than copy the explanation.
...
For **Modern extent filesystems**, identify the exact state stored by the kernel, who can change it, what event causes progress, and what other subsystem must cooperate.
