# 65.11 NUMA-aware allocators

## Core idea

NUMA systems have multiple memory nodes with different local/remote costs. Thread affinity and allocation placement therefore become coupled performance decisions.

This is the canonical note for **NUMA-aware allocators** inside **NUMA and Memory Locality**. Other OS notes should link here rather than copy the explanation.
...
For **NUMA-aware allocators**, identify the exact state stored by the kernel, who can change it, what event causes progress, and what other subsystem must cooperate.
