# 39.03 Banker's algorithm

## Core idea

Deadlock avoidance keeps the system in a safe state. Banker's algorithm models maximum resource demand and tests whether a safe completion order exists.

This is the canonical note for **Banker's algorithm** inside **Deadlock Avoidance and Banker's Algorithm**. Other OS notes should link here rather than copy the explanation.
...
For **Banker's algorithm**, identify the exact state stored by the kernel, who can change it, what event causes progress, and what other subsystem must cooperate.
