# 67.12 File-system layering

## Core idea

A filesystem turns block storage into files, directories, metadata, allocation structures and consistency rules.

This is the canonical note for **File-system layering** inside **File System Fundamentals**. Other OS notes should link here rather than copy the explanation.

## Mental model

```text
application -> VFS -> filesystem -> block layer -> device
```
...
For **File-system layering**, identify the exact state stored by the kernel, who can change it, what event causes progress, and what other subsystem must cooperate.
