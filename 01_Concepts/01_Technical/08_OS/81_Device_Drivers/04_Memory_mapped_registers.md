# 81.04 Memory-mapped registers

## Core idea

Drivers translate OS interfaces into device-specific operations and must manage initialization, interrupts, DMA, synchronization, power and error recovery.

This is the canonical note for **Memory-mapped registers** inside **Device Drivers**. Other OS notes should link here rather than copy the explanation.
...
For **Memory-mapped registers**, identify the exact state stored by the kernel, who can change it, what event causes progress, and what other subsystem must cooperate.
