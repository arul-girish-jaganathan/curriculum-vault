# 62.10 Shared-memory use

## Core idea

Memory mapping creates a virtual-memory relationship to files, anonymous storage or devices. Shared and private mappings have different write visibility semantics.

This is the canonical note for **Shared-memory use** inside **Memory-Mapped Files**. Other OS notes should link here rather than copy the explanation.
...
For **Shared-memory use**, identify the exact state stored by the kernel, who can change it, what event causes progress, and what other subsystem must cooperate.
