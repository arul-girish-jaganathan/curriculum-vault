# 14.07 File descriptor inheritance

## Core idea

fork duplicates a process execution context, commonly using copy-on-write for the address space, while exec replaces the process image. wait synchronizes parent supervision with child completion.

This is the canonical note for **File descriptor inheritance** inside **fork, exec, wait and Process Replacement**. Other OS notes should link here rather than copy the explanation.
...
For **File descriptor inheritance**, identify the exact state stored by the kernel, who can change it, what event causes progress, and what other subsystem must cooperate.
