# 06.01 System-call interface

## Core idea

A system call is a controlled transition from user execution into a kernel service. The ABI defines arguments, calling convention, privilege transition and result/error semantics.

This is the canonical note for **System-call interface** inside **System Calls and APIs**. Other OS notes should link here rather than copy the explanation.

## Mental model

```text
user mode -> syscall/trap -> kernel service -> return to user
```
...
For **System-call interface**, identify the exact state stored by the kernel, who can change it, what event causes progress, and what other subsystem must cooperate.
