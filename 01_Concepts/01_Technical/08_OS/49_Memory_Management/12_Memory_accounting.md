# 49.12 Memory accounting

## Core idea

OS memory management allocates, protects and maps memory while allowing many processes to coexist. Current GFG coverage includes allocation, protection, swapping and virtual memory. citeturn388804search7

This is the canonical note for **Memory accounting** inside **Memory Management Fundamentals**. Other OS notes should link here rather than copy the explanation.

## Mental model

```text
virtual address -> page tables/TLB -> physical memory
```
...
For **Memory accounting**, identify the exact state stored by the kernel, who can change it, what event causes progress, and what other subsystem must cooperate.
