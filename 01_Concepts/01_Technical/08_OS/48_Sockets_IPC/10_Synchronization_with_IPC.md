# 48.10 Synchronization with IPC

## Core idea

IPC lets independent processes exchange data using message passing, shared memory, pipes, signals, sockets and other kernel-mediated mechanisms. citeturn388804search9

This is the canonical note for **Synchronization with IPC** inside **Sockets and Local IPC**. Other OS notes should link here rather than copy the explanation.
...
For **Synchronization with IPC**, identify the exact state stored by the kernel, who can change it, what event causes progress, and what other subsystem must cooperate.
