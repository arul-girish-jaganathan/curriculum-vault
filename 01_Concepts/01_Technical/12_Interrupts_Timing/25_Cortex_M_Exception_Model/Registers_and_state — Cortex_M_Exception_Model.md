# Registers and state — Cortex M Exception Model

## Concept
**Registers and state** in the context of **Cortex M Exception Model** should be understood from the observable event through the final required response. Keep separate the hardware event, controller state, CPU/OS entry, handler execution, deferred work, and deadline completion.
