# Hardware signal path — ISR Execution Time

## Concept
**Hardware signal path** in the context of **ISR Execution Time** should be understood from the observable event through the final required response. Keep separate the hardware event, controller state, CPU/OS entry, handler execution, deferred work, and deadline completion.
