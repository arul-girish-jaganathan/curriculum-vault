# Trigger and status bits — CAN Interrupts

## Concept
**Trigger and status bits** in the context of **CAN Interrupts** should be understood from the observable event through the final required response. Keep separate the hardware event, controller state, CPU/OS entry, handler execution, deferred work, and deadline completion.
