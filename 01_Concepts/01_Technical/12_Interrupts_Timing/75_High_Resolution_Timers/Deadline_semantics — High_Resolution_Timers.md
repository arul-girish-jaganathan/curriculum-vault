# Deadline semantics — High Resolution Timers

## Concept
**Deadline semantics** in the context of **High Resolution Timers** should be understood from the observable event through the final required response. Keep separate the hardware event, controller state, CPU/OS entry, handler execution, deferred work, and deadline completion.
