# Hardware signal path — Pulse Width and Lost Interrupts

## Concept
**Hardware signal path** in the context of **Pulse Width and Lost Interrupts** should be understood from the observable event through the final required response. Keep separate the hardware event, controller state, CPU/OS entry, handler execution, deferred work, and deadline completion.
