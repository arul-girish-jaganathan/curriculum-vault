# Kernel path — Staff Level Interrupt Timing Architecture

## Concept
**Kernel path** in the context of **Staff Level Interrupt Timing Architecture** should be understood from the observable event through the final required response. Keep separate the hardware event, controller state, CPU/OS entry, handler execution, deferred work, and deadline completion.
