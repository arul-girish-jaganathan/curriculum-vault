# Load interactions — Real Time Linux PREEMPT RT

## Concept
**Load interactions** in the context of **Real Time Linux PREEMPT RT** should be understood from the observable event through the final required response. Keep separate the hardware event, controller state, CPU/OS entry, handler execution, deferred work, and deadline completion.
