# Hardware signal path — Pending Active State

## Concept
**Hardware signal path** in the context of **Pending Active State** should be understood from the observable event through the final required response. Keep separate the hardware event, controller state, CPU/OS entry, handler execution, deferred work, and deadline completion.
