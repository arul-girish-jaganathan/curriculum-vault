# Hardware signal path — Spurious and False Interrupts

## Concept
**Hardware signal path** in the context of **Spurious and False Interrupts** should be understood from the observable event through the final required response. Keep separate the hardware event, controller state, CPU/OS entry, handler execution, deferred work, and deadline completion.
