# Kernel path — Interrupt Timing Testing and Qualification

## Concept
**Kernel path** in the context of **Interrupt Timing Testing and Qualification** should be understood from the observable event through the final required response. Keep separate the hardware event, controller state, CPU/OS entry, handler execution, deferred work, and deadline completion.
