# Driver state machine — ADC Interrupts

## Concept
**Driver state machine** in the context of **ADC Interrupts** should be understood from the observable event through the final required response. Keep separate the hardware event, controller state, CPU/OS entry, handler execution, deferred work, and deadline completion.
