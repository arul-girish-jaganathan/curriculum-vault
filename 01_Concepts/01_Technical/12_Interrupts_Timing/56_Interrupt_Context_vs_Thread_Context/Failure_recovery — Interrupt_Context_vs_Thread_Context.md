# Failure recovery — Interrupt Context vs Thread Context

## Concept
**Failure recovery** in the context of **Interrupt Context vs Thread Context** should be understood from the observable event through the final required response. Keep separate the hardware event, controller state, CPU/OS entry, handler execution, deferred work, and deadline completion.
