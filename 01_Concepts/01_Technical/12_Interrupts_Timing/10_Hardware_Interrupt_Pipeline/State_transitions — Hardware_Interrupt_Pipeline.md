# State transitions — Hardware Interrupt Pipeline

## Concept
**State transitions** in the context of **Hardware Interrupt Pipeline** should be understood from the observable event through the final required response. Keep separate the hardware event, controller state, CPU/OS entry, handler execution, deferred work, and deadline completion.
