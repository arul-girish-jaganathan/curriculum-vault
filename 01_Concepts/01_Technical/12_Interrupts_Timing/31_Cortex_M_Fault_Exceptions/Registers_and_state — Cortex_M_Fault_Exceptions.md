# Registers and state — Cortex M Fault Exceptions

## Concept
**Registers and state** in the context of **Cortex M Fault Exceptions** should be understood from the observable event through the final required response. Keep separate the hardware event, controller state, CPU/OS entry, handler execution, deferred work, and deadline completion.
