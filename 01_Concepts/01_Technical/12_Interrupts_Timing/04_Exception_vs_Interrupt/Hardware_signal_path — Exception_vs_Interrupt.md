# Hardware signal path — Exception vs Interrupt

## Concept
**Hardware signal path** in the context of **Exception vs Interrupt** should be understood from the observable event through the final required response. Keep separate the hardware event, controller state, CPU/OS entry, handler execution, deferred work, and deadline completion.
