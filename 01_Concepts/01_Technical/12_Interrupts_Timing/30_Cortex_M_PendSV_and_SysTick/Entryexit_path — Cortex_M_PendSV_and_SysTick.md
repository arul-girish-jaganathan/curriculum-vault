# Entry/exit path — Cortex M PendSV and SysTick

## Concept
**Entry/exit path** in the context of **Cortex M PendSV and SysTick** should be understood from the observable event through the final required response. Keep separate the hardware event, controller state, CPU/OS entry, handler execution, deferred work, and deadline completion.
