# Synchronization — ISR Driven State Machines

## Concept
**Synchronization** in the context of **ISR Driven State Machines** should be understood from the observable event through the final required response. Keep separate the hardware event, controller state, CPU/OS entry, handler execution, deferred work, and deadline completion.
