# Entry/exit path — FIQ NMI and High Priority Events

## Concept
**Entry/exit path** in the context of **FIQ NMI and High Priority Events** should be understood from the observable event through the final required response. Keep separate the hardware event, controller state, CPU/OS entry, handler execution, deferred work, and deadline completion.
