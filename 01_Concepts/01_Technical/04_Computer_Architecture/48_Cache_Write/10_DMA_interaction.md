# 48.10 DMA interaction

## Core Idea

**DMA interaction** is a canonical subtopic inside **Write-Through, Write-Back and Write Policies**. The goal is to understand not just the definition, but what state is involved, how the hardware or control logic behaves, what timing/resource effects result, and what failure modes an engineer must recognize.

## Mechanism

```text
CPU configures DMA -> DMA owns transfer -> memory/peripheral transfer -> completion interrupt
```
