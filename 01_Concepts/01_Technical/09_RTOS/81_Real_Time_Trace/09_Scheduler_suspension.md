# 81.09 Scheduler suspension

## Core idea

A real-time scheduler must select the task that should run according to its policy and make that selection with bounded overhead. Preemptive priority scheduling is a common RTOS model. citeturn331504search2

This is the canonical note for **Scheduler suspension** within **Real-Time Trace and Scheduling Visualization**. Other RTOS notes should link here instead of repeating the same explanation.

## Mental model

```text
highest-priority ready task -> CPU
```
