# 61.12 Memory instrumentation

## Core idea

RTOS memory includes stacks, kernel objects, application buffers and heaps/pools. Static and dynamic approaches trade predictability against flexibility. FreeRTOS, for example, supports both static and dynamic creation for several kernel objects. citeturn381180search0

This is the canonical note for **Memory instrumentation** within **Heap Corruption and Fragmentation**. Other RTOS notes should link here instead of repeating the same explanation.

## Mental model

```text
stack + kernel objects + heap/pools + application buffers
```
