# 86.12 Kernel configuration

## Core idea

Zephyr provides threads, scheduling, synchronization, data-passing objects, timers, workqueues, SMP and other kernel services. Its current documentation explicitly groups scheduling, interrupts, semaphores, mutexes, events and data-passing primitives. citeturn381180search2

This is the canonical note for **Kernel configuration** within **Zephyr Kernel Concepts**. Other RTOS notes should link here instead of repeating the same explanation.

## Mental model

```text
thread + scheduler + IPC + sync + timers + workqueue
```
