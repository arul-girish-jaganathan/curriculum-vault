# 05.03 Interrupt-driven firmware

## Core idea

An RTOS adds structured concurrency and kernel services to a bare-metal system. The decision should be driven by execution complexity, timing requirements, isolation and maintenance cost rather than by fashion.

This is the canonical note for **Interrupt-driven firmware** within **RTOS versus Bare-Metal Architecture**. Other RTOS notes should link here instead of repeating the same explanation.

## Mental model

```text
superloop -> interrupt -> service routines
RTOS: tasks -> scheduler -> IPC/sync -> drivers
```
