# 02.12 Requirement traceability

## Core idea

Real-time requirements are expressed as deadlines, periods, response-time limits, jitter budgets and resource bounds. A system is only as good as the assumptions used to define and verify those limits.

This is the canonical note for **Requirement traceability** within **Real-Time Requirements, Deadlines and Guarantees**. Other RTOS notes should link here instead of repeating the same explanation.

## Mental model

```text
release -> execution -> blocking -> response <= deadline
```
