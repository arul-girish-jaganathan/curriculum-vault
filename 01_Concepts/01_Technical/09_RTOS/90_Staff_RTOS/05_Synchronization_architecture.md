# 90.05 Synchronization architecture

## Core idea

Staff-level RTOS engineering connects product deadlines and resource budgets to task architecture, priorities, synchronization, fault handling, observability and verification.

This is the canonical note for **Synchronization architecture** within **Staff-Level RTOS Architecture and Design**. Other RTOS notes should link here instead of repeating the same explanation.

## Mental model

```text
requirements -> tasks -> priorities -> sync -> timing -> recovery -> evidence
```
