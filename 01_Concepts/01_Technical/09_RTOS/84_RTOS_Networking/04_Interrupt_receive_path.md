# 84.04 Interrupt receive path

## Core idea

Networking in an RTOS is usually a pipeline of interrupt/DMA reception, buffers, protocol processing and application tasks. Queue depth and backpressure determine behavior under bursts.

This is the canonical note for **Interrupt receive path** within **Networking in RTOS Systems**. Other RTOS notes should link here instead of repeating the same explanation.

## Mental model

```text
IRQ/DMA -> RX buffer -> network task -> protocol -> application
```
