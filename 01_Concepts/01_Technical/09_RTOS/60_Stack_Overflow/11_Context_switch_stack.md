# 60.11 Context switch stack

## Core idea

Each task requires a stack large enough for its worst-case call depth and context requirements. Stack overflow is a particularly dangerous failure because corruption can precede the visible crash.

This is the canonical note for **Context switch stack** within **Stack Overflow Detection**. Other RTOS notes should link here instead of repeating the same explanation.

## Mental model

```text
stack top -> saved context -> locals -> call frames
```
