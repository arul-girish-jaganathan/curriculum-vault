# 75.10 Latency versus power

## Core idea

An RTOS can reduce energy by sleeping when no required work is runnable, but wake latency, timer programming and peripheral state restoration become part of the timing contract.

This is the canonical note for **Latency versus power** within **Low-Power RTOS Design**. Other RTOS notes should link here instead of repeating the same explanation.

## Mental model

```text
no runnable work -> sleep -> wake -> restore timing -> run
```
