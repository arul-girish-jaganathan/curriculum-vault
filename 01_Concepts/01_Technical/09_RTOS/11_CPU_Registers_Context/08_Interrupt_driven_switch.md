# 11.08 Interrupt-driven switch

## Core idea

A context switch saves enough execution state to resume one task and restores another. On Cortex-M, SVC/PendSV/SysTick are commonly used mechanisms in RTOS ports; PendSV is especially suited to deferred context switching. citeturn381180search7

This is the canonical note for **Interrupt-driven switch** within **CPU Context and Register Save/Restore**. Other RTOS notes should link here instead of repeating the same explanation.

## Mental model

```text
save current context -> choose next -> restore next context -> return
```
