# 51.09 Timer cancellation

## Core idea

RTOS timers convert time into callbacks or events. Their precision depends on the time base, timer service architecture and scheduling latency.

This is the canonical note for **Timer cancellation** within **Software Timers**. Other RTOS notes should link here instead of repeating the same explanation.

## Mental model

```text
timer expiry -> timer service context -> callback/event -> task
```
