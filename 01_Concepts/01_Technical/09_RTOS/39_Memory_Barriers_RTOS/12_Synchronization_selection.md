# 39.12 Synchronization selection

## Core idea

RTOS synchronization provides mutual exclusion or coordination, but each primitive has different ownership, blocking, priority and ISR-use semantics.

This is the canonical note for **Synchronization selection** within **Memory Barriers and RTOS Synchronization**. Other RTOS notes should link here instead of repeating the same explanation.

## Mental model

```text
task A --lock/event--> shared state <--lock/event-- task B
```
