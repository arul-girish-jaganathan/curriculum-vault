# 85.12 Real-time limitations

## Core idea

Linux can provide real-time behavior through PREEMPT_RT and careful kernel/application design, but its latency characteristics differ from a small MCU RTOS and must be measured under target workload.

This is the canonical note for **Real-time limitations** within **Real-Time Linux Concepts**. Other RTOS notes should link here instead of repeating the same explanation.

## Mental model

```text
IRQ/thread -> kernel scheduler -> PREEMPT_RT thread -> application
```
