# 71.04 Per-core ready queues

## Core idea

SMP/AMP RTOS systems add per-core scheduling, inter-core interrupts, cache coherence and shared-memory synchronization.

This is the canonical note for **Per-core ready queues** within **AMP and Heterogeneous Processing**. Other RTOS notes should link here instead of repeating the same explanation.

## Mental model

```text
CPU0 task <-> shared memory <-> CPU1 task
          \-> IPI / synchronization
```
