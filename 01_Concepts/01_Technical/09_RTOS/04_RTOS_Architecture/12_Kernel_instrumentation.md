# 04.12 Kernel instrumentation

## Core idea

The RTOS kernel arbitrates CPU execution and provides synchronization, timing and communication services. Its APIs are only the surface; scheduler data structures and context-switch paths determine actual behavior.

This is the canonical note for **Kernel instrumentation** within **RTOS Kernel Architecture**. Other RTOS notes should link here instead of repeating the same explanation.
...
RTOS API
    ↓
scheduler / IPC / sync / timer / memory
    ↓
CPU + interrupts + hardware
```
