# 82.01 DMA transfer ownership

## Core idea

DMA interacts with the RTOS as another execution agent. The ownership of buffers must transfer cleanly between task, ISR, DMA and cache/memory system.

This is the canonical note for **DMA transfer ownership** within **Drivers in an RTOS**. Other RTOS notes should link here instead of repeating the same explanation.

## Mental model

```text
DMA owns buffer -> completion IRQ -> task notified -> task consumes -> recycle
```
