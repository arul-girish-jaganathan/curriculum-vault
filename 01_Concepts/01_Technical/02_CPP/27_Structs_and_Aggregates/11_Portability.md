...
**Portability** is a concrete part of **Structs and Aggregate Initialization**. The topic belongs to the C++ abstract machine and program-building model. Syntax is useful only when the resulting semantics are understood.
...
Advanced C++ work requires identifying the boundary between standard language semantics and implementation detail. Record the compiler, standard library and ABI assumptions that matter. The specific question for **Portability** in **27 Structs and Aggregates** is to make this rule explicit in the code and documentation rather than relying on convention.

### Mechanism detail

Binary compatibility can break even when source code still compiles. Layout, name mangling, exception ABI and library versions all matter across module boundaries. The specific question for **Portability** in **27 Structs and Aggregates** is to make this rule explicit in the code and documentation rather than relying on convention.

### Design implication

Modules and modern build systems change dependency structure but still need a reproducible compiler and toolchain configuration. The specific question for **Portability** in **27 Structs and Aggregates** is to make this rule explicit in the code and documentation rather than relying on convention.

### Failure analysis

Coroutines transform control flow and create explicit suspension/resumption points. Lifetime of captured state is therefore part of the design. The specific question for **Portability** in **27 Structs and Aggregates** is to make this rule explicit in the code and documentation rather than relying on convention.

### Portability note

Toolchain reproducibility includes compiler version, flags, standard library, linker inputs and generated sources. These belong in the build artifact's provenance. The specific question for **Portability** in **27 Structs and Aggregates** is to make this rule explicit in the code and documentation rather than relying on convention.

### Resource analysis

Performance claims should be supported by target measurements or generated-code inspection when they affect architecture. The specific question for **Portability** in **27 Structs and Aggregates** is to make this rule explicit in the code and documentation rather than relying on convention.

### Debugging evidence

Static analysis and sanitizers reveal different classes of defects; using both provides better coverage than relying on either alone. The specific question for **Portability** in **27 Structs and Aggregates** is to make this rule explicit in the code and documentation rather than relying on convention.

### Verification ideas

Release engineering is part of software correctness. The exact binary, symbols, map file and source revision should be traceable after shipment. The specific question for **Portability** in **27 Structs and Aggregates** is to make this rule explicit in the code and documentation rather than relying on convention.

### Embedded scenario

Migration planning should consider language-standard support, compiler diagnostics, library support and coding guidelines—not just whether the feature is syntactically available. The specific question for **Portability** in **27 Structs and Aggregates** is to make this rule explicit in the code and documentation rather than relying on convention.

### Expert review

Expert review asks what happens after the platform changes. A design that depends on undocumented behavior is carrying migration debt whether or not it has failed yet. The specific question for **Portability** in **27 Structs and Aggregates** is to make this rule explicit in the code and documentation rather than relying on convention.
