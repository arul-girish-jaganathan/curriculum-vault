# 72.11 Error handling for I/O

## Core idea

**Error handling for I/O** is a concrete part of **File Streams and File I/O**. I/O is interaction with an external stateful resource. Buffering, partial operations, failure states and resource lifetime must be explicit.
...
Failure handling starts by defining what constitutes failure. A zero value, empty result or special status may be valid data rather than an error. The specific question for **Error handling for I/O** in **72 File IO** is to make this rule explicit in the code and documentation rather than relying on convention.

### Mechanism detail

Return-code APIs work best when status and output data are clearly separated and the caller cannot accidentally treat an error result as valid output. The specific question for **Error handling for I/O** in **72 File IO** is to make this rule explicit in the code and documentation rather than relying on convention.
...
Cleanup should happen regardless of where failure occurs. RAII is the standard C++ mechanism for making release behavior structural. The specific question for **Error handling for I/O** in **72 File IO** is to make this rule explicit in the code and documentation rather than relying on convention.
...
Assertions are best for invariants and programmer assumptions that indicate a defect, not for expected runtime input validation. The specific question for **Error handling for I/O** in **72 File IO** is to make this rule explicit in the code and documentation rather than relying on convention.
...
Fault containment reduces the blast radius. A malformed message should not necessarily be able to corrupt unrelated subsystem state. The specific question for **Error handling for I/O** in **72 File IO** is to make this rule explicit in the code and documentation rather than relying on convention.
