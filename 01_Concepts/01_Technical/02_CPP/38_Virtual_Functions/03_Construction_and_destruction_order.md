# 38.03 Construction and destruction order

## Core idea

**Construction and destruction order** is a concrete part of **Virtual Functions and Dynamic Dispatch**. Inheritance relates base and derived subobjects. Correctness depends on substitutability, initialization/destruction order, name lookup, overriding and the chosen polymorphism model.
...
Boundary cases should be tested early because they expose assumptions that the simple example hides. The specific question for **Construction and destruction order** in **38 Virtual Functions** is to make this rule explicit in the code and documentation rather than relying on convention.

### Failure analysis

Portability requires distinguishing standard guarantees from implementation-defined and unspecified behavior. The specific question for **Construction and destruction order** in **38 Virtual Functions** is to make this rule explicit in the code and documentation rather than relying on convention.
...
Use tests to encode the behavior the team relies on. If a property matters, make it executable evidence. The specific question for **Construction and destruction order** in **38 Virtual Functions** is to make this rule explicit in the code and documentation rather than relying on convention.
