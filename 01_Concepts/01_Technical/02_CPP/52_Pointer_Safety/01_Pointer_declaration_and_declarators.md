# 52.01 Pointer declaration and declarators

## Core idea

**Pointer declaration and declarators** is a concrete part of **Pointer Lifetime, Ownership and Aliasing**. Pointers model indirection. Correctness depends on pointed-to type, validity, object lifetime, alignment, ownership and the context in which the address is used.
...
A pointer may be copied freely while its validity does not become stronger. Every copy inherits the same lifetime constraints. The specific question for **Pointer declaration and declarators** in **52 Pointer Safety** is to make this rule explicit in the code and documentation rather than relying on convention.
