# 78.03 Error propagation

## Core idea

**Error propagation** is a concrete part of **Error Codes and Exception-Free Error Policy**. Failure handling is a system policy: detect the problem, preserve useful state, clean up, propagate meaningfully, and recover or fail safely.
...
Failure handling starts by defining what constitutes failure. A zero value, empty result or special status may be valid data rather than an error. The specific question for **Error propagation** in **78 Error Handling** is to make this rule explicit in the code and documentation rather than relying on convention.
...
Cleanup should happen regardless of where failure occurs. RAII is the standard C++ mechanism for making release behavior structural. The specific question for **Error propagation** in **78 Error Handling** is to make this rule explicit in the code and documentation rather than relying on convention.
...
Assertions are best for invariants and programmer assumptions that indicate a defect, not for expected runtime input validation. The specific question for **Error propagation** in **78 Error Handling** is to make this rule explicit in the code and documentation rather than relying on convention.
...
Fault containment reduces the blast radius. A malformed message should not necessarily be able to corrupt unrelated subsystem state. The specific question for **Error propagation** in **78 Error Handling** is to make this rule explicit in the code and documentation rather than relying on convention.
