# 88.01 Memory-mapped I/O

## Core idea

**Memory-mapped I/O** is a concrete part of **Embedded C++ Patterns**. Embedded C++ is constrained by hardware, timing, memory, interrupts, DMA, caches, ABI and safety requirements. The language feature is only useful when those costs are bounded.
...
Container semantics are defined by storage strategy, access pattern and iterator/reference stability. Member-function names alone do not tell the whole performance story. The specific question for **Memory-mapped I/O** in **88 Embedded Cpp** is to make this rule explicit in the code and documentation rather than relying on convention.

### Mechanism detail

Contiguous storage improves locality and works naturally with hardware buffers, but insertion that reallocates may invalidate retained addresses. The specific question for **Memory-mapped I/O** in **88 Embedded Cpp** is to make this rule explicit in the code and documentation rather than relying on convention.

### Design implication

Node-based containers provide different stability properties while paying for individual nodes and pointer chasing. The specific question for **Memory-mapped I/O** in **88 Embedded Cpp** is to make this rule explicit in the code and documentation rather than relying on convention.

### Failure analysis

Associative containers trade a different complexity profile and memory shape against ordered lookup. For small collections, a simple sorted array can be a better engineering choice. The specific question for **Memory-mapped I/O** in **88 Embedded Cpp** is to make this rule explicit in the code and documentation rather than relying on convention.

### Portability note

Unordered containers add hashing, buckets and possible rehashing. Average constant-time lookup does not mean constant memory or constant worst-case latency. The specific question for **Memory-mapped I/O** in **88 Embedded Cpp** is to make this rule explicit in the code and documentation rather than relying on convention.

### Resource analysis

Container capacity is distinct from size. Predicting maximum capacity can avoid unnecessary reallocations and make memory budgets explicit. The specific question for **Memory-mapped I/O** in **88 Embedded Cpp** is to make this rule explicit in the code and documentation rather than relying on convention.

### Debugging evidence

Iterator invalidation is part of the API contract. A cached iterator that becomes invalid after insertion or reallocation is a correctness defect. The specific question for **Memory-mapped I/O** in **88 Embedded Cpp** is to make this rule explicit in the code and documentation rather than relying on convention.

### Verification ideas

Algorithms are most useful when the container semantics already match the workload. Replacing a clear loop with an algorithm is not valuable if it obscures a critical timing property. The specific question for **Memory-mapped I/O** in **88 Embedded Cpp** is to make this rule explicit in the code and documentation rather than relying on convention.

### Embedded scenario

Allocator behavior can dominate the real cost in dynamic containers. Measure allocation count and failure behavior, not only element-operation complexity. The specific question for **Memory-mapped I/O** in **88 Embedded Cpp** is to make this rule explicit in the code and documentation rather than relying on convention.
