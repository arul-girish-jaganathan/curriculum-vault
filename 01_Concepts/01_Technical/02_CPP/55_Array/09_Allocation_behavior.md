# 55.09 Allocation behavior

## Core idea

**Allocation behavior** is a concrete part of **std::array and Fixed-Size Data**. Containers define storage and access policy. Size, capacity, locality, allocation and invalidation rules are as important as the member-function names.
...
Memory correctness requires separate answers for storage, object lifetime and ownership. A valid address does not prove that a valid object still exists there. The specific question for **Allocation behavior** in **55 Array** is to make this rule explicit in the code and documentation rather than relying on convention.

### Mechanism detail

Allocation and construction are related but conceptually different. Likewise destruction and deallocation are different steps in the lifetime model. The specific question for **Allocation behavior** in **55 Array** is to make this rule explicit in the code and documentation rather than relying on convention.
...
Ownership must be unique, shared or borrowed by design. Ambiguous ownership is a common root cause of leaks and double-destruction defects. The specific question for **Allocation behavior** in **55 Array** is to make this rule explicit in the code and documentation rather than relying on convention.
...
Alignment is a correctness requirement, not just a micro-optimization. Packed or manually constructed objects can violate target access requirements. The specific question for **Allocation behavior** in **55 Array** is to make this rule explicit in the code and documentation rather than relying on convention.
...
Low-memory recovery is itself a resource problem. A system should know what it can still do safely after an allocation failure. The specific question for **Allocation behavior** in **55 Array** is to make this rule explicit in the code and documentation rather than relying on convention.
