# 12.03 Comparison operators

## Core idea

**Comparison operators** is a concrete part of **Operators, Precedence and Side Effects**. Expressions are typed computations with precedence, conversions and side effects. Operator behavior must be understood semantically, not just by visual appearance.
...
Operator expressions are governed by precedence, associativity, conversions and evaluation rules. Visual similarity to mathematics is not enough to determine behavior. The specific question for **Comparison operators** in **12 Operators** is to make this rule explicit in the code and documentation rather than relying on convention.

### Mechanism detail

Short-circuit logical operators are valuable for guarding dependent operations because the right operand may not be evaluated. The specific question for **Comparison operators** in **12 Operators** is to make this rule explicit in the code and documentation rather than relying on convention.

### Design implication

Increment and decrement introduce side effects and should be used where the resulting order is obvious. The specific question for **Comparison operators** in **12 Operators** is to make this rule explicit in the code and documentation rather than relying on convention.

### Failure analysis

Overloaded operators should preserve intuitive meaning. A `+` that allocates, locks or mutates unrelated state creates surprise and hidden cost. The specific question for **Comparison operators** in **12 Operators** is to make this rule explicit in the code and documentation rather than relying on convention.

### Portability note

Comparison operators should establish a coherent relation that works with the algorithms or containers that consume them. The specific question for **Comparison operators** in **12 Operators** is to make this rule explicit in the code and documentation rather than relying on convention.

### Resource analysis

Bitwise operators are central to masks and packed fields. Signedness and shift rules need explicit review. The specific question for **Comparison operators** in **12 Operators** is to make this rule explicit in the code and documentation rather than relying on convention.

### Debugging evidence

Operator precedence mistakes can survive testing because one input pattern happens to produce the expected value. Parentheses are a low-cost correctness aid. The specific question for **Comparison operators** in **12 Operators** is to make this rule explicit in the code and documentation rather than relying on convention.

### Verification ideas

Comma and conditional operators are useful but can reduce readability when they compress multiple effects into a single expression. The specific question for **Comparison operators** in **12 Operators** is to make this rule explicit in the code and documentation rather than relying on convention.

### Embedded scenario

Expression side effects become harder to analyze when several updates are combined. Prefer simple expressions in safety- or timing-critical code. The specific question for **Comparison operators** in **12 Operators** is to make this rule explicit in the code and documentation rather than relying on convention.

### Expert review

Generated assembly or a targeted unit test is useful when a subtle expression is part of a performance or hardware-critical path. The specific question for **Comparison operators** in **12 Operators** is to make this rule explicit in the code and documentation rather than relying on convention.
