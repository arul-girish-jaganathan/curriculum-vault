...
**Data races** is a concrete part of **Assertions, Preconditions and Contract Thinking**. Concurrent code must be reasoned about through possible interleavings, synchronization, visibility, ownership and progress rather than through one observed execution.
...
Concurrent correctness depends on all legal interleavings, not only the interleaving observed in a passing test. The specific question for **Data races** in **25 Assertions and Contracts** is to make this rule explicit in the code and documentation rather than relying on convention.

### Mechanism detail

Thread lifetime is an ownership problem. A thread must have a defined owner, completion policy and relationship to resources it accesses. The specific question for **Data races** in **25 Assertions and Contracts** is to make this rule explicit in the code and documentation rather than relying on convention.

### Design implication

Protect invariants, not merely variables. If two fields must change together, they usually belong under the same synchronization policy. The specific question for **Data races** in **25 Assertions and Contracts** is to make this rule explicit in the code and documentation rather than relying on convention.

### Failure analysis

RAII locking prevents many early-return mistakes, but it does not choose correct lock scope or eliminate deadlock. The specific question for **Data races** in **25 Assertions and Contracts** is to make this rule explicit in the code and documentation rather than relying on convention.

### Portability note

Condition variables wait for predicates. A wakeup is not the condition itself and must not be treated as proof that the predicate is true. The specific question for **Data races** in **25 Assertions and Contracts** is to make this rule explicit in the code and documentation rather than relying on convention.

### Resource analysis

Atomics solve one class of data races but do not automatically make a multi-step protocol atomic or logically race-free. The specific question for **Data races** in **25 Assertions and Contracts** is to make this rule explicit in the code and documentation rather than relying on convention.

### Debugging evidence

Memory order determines visibility and synchronization relationships. Use relaxed ordering only when the algorithm remains correct without stronger ordering. The specific question for **Data races** in **25 Assertions and Contracts** is to make this rule explicit in the code and documentation rather than relying on convention.

### Verification ideas

Detached execution complicates shutdown because the creator cannot simply wait for completion before destroying shared state. The specific question for **Data races** in **25 Assertions and Contracts** is to make this rule explicit in the code and documentation rather than relying on convention.

### Embedded scenario

Timing analysis must account for blocking, priority inversion, wakeup latency and scheduler behavior, not just the CPU cost of the protected code. The specific question for **Data races** in **25 Assertions and Contracts** is to make this rule explicit in the code and documentation rather than relying on convention.

### Expert review

Concurrency bugs benefit from stress tests that vary scheduling, CPU affinity, message timing and shutdown order rather than merely running more input vectors. The specific question for **Data races** in **25 Assertions and Contracts** is to make this rule explicit in the code and documentation rather than relying on convention.
