# 37.05 Overriding virtual functions

## Core idea

**Overriding virtual functions** is a concrete part of **Inheritance and Derived Classes**. Inheritance relates base and derived subobjects. Correctness depends on substitutability, initialization/destruction order, name lookup, overriding and the chosen polymorphism model.
...
Public inheritance is a semantic promise of substitutability, not merely a code-reuse mechanism. The derived type must preserve the expectations established by the base abstraction. The specific question for **Overriding virtual functions** in **37 Inheritance** is to make this rule explicit in the code and documentation rather than relying on convention.

### Mechanism detail

Construction and destruction order is fixed. A derived implementation cannot safely assume that a derived member exists while the base constructor is executing. The specific question for **Overriding virtual functions** in **37 Inheritance** is to make this rule explicit in the code and documentation rather than relying on convention.

### Design implication

Name hiding and virtual overriding are separate mechanisms. A function can hide a base overload even when no virtual dispatch is involved. The specific question for **Overriding virtual functions** in **37 Inheritance** is to make this rule explicit in the code and documentation rather than relying on convention.

### Failure analysis

Multiple inheritance changes object layout and can introduce repeated base subobjects or ambiguity. The design should make each base relationship semantically necessary. The specific question for **Overriding virtual functions** in **37 Inheritance** is to make this rule explicit in the code and documentation rather than relying on convention.

### Portability note

A polymorphic base normally needs a suitable virtual destruction contract when clients may destroy derived objects through a base pointer. The specific question for **Overriding virtual functions** in **37 Inheritance** is to make this rule explicit in the code and documentation rather than relying on convention.

### Resource analysis

Object slicing occurs when a derived object is copied into a base value. It removes the derived state and is frequently an API-design mistake in polymorphic systems. The specific question for **Overriding virtual functions** in **37 Inheritance** is to make this rule explicit in the code and documentation rather than relying on convention.

### Debugging evidence

Dynamic casting can provide runtime relationship checks but should not become the normal way an architecture discovers which implementation it is dealing with. The specific question for **Overriding virtual functions** in **37 Inheritance** is to make this rule explicit in the code and documentation rather than relying on convention.

### Verification ideas

Virtual dispatch has a target-level cost and ABI representation, but the deeper cost is often architectural: ownership and lifecycle become more complex when objects are manipulated through base interfaces. The specific question for **Overriding virtual functions** in **37 Inheritance** is to make this rule explicit in the code and documentation rather than relying on convention.

### Embedded scenario

Prefer composition when the relationship is ownership or implementation reuse rather than substitutability. This often keeps embedded designs flatter and easier to verify. The specific question for **Overriding virtual functions** in **37 Inheritance** is to make this rule explicit in the code and documentation rather than relying on convention.

### Expert review

Review a hierarchy by drawing actual subobjects and call targets. Then separately reason about construction, lookup, dispatch and destruction. The specific question for **Overriding virtual functions** in **37 Inheritance** is to make this rule explicit in the code and documentation rather than relying on convention.
