# 59.10 Allocation failure

## Core idea

**Allocation failure** is a concrete part of **Allocators and Memory Resources**. Memory management is a lifecycle problem: storage acquisition, object construction, ownership, object lifetime, destruction and storage release are distinct concepts.
...
Ownership must be unique, shared or borrowed by design. Ambiguous ownership is a common root cause of leaks and double-destruction defects. The specific question for **Allocation failure** in **59 Allocators** is to make this rule explicit in the code and documentation rather than relying on convention.
...
Low-memory recovery is itself a resource problem. A system should know what it can still do safely after an allocation failure. The specific question for **Allocation failure** in **59 Allocators** is to make this rule explicit in the code and documentation rather than relying on convention.
