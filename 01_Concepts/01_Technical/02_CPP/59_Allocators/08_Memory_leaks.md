...
**Memory leaks** is a concrete part of **Allocators and Memory Resources**. Memory management is a lifecycle problem: storage acquisition, object construction, ownership, object lifetime, destruction and storage release are distinct concepts.
...
Memory correctness requires separate answers for storage, object lifetime and ownership. A valid address does not prove that a valid object still exists there. The specific question for **Memory leaks** in **59 Allocators** is to make this rule explicit in the code and documentation rather than relying on convention.

### Mechanism detail

Allocation and construction are related but conceptually different. Likewise destruction and deallocation are different steps in the lifetime model. The specific question for **Memory leaks** in **59 Allocators** is to make this rule explicit in the code and documentation rather than relying on convention.

### Design implication

Automatic storage is convenient and deterministic but constrained by stack depth and object size. Large local arrays can become a field failure on small targets. The specific question for **Memory leaks** in **59 Allocators** is to make this rule explicit in the code and documentation rather than relying on convention.

### Failure analysis

Dynamic allocation introduces runtime failure and potentially fragmentation. A system using it for long-lived services needs an explicit allocation policy. The specific question for **Memory leaks** in **59 Allocators** is to make this rule explicit in the code and documentation rather than relying on convention.

### Portability note

Ownership must be unique, shared or borrowed by design. Ambiguous ownership is a common root cause of leaks and double-destruction defects. The specific question for **Memory leaks** in **59 Allocators** is to make this rule explicit in the code and documentation rather than relying on convention.

### Resource analysis

Copy operations can accidentally duplicate resource handles. Move operations can transfer ownership but leave the source object in a valid yet often unspecified state. The specific question for **Memory leaks** in **59 Allocators** is to make this rule explicit in the code and documentation rather than relying on convention.

### Debugging evidence

Alignment is a correctness requirement, not just a micro-optimization. Packed or manually constructed objects can violate target access requirements. The specific question for **Memory leaks** in **59 Allocators** is to make this rule explicit in the code and documentation rather than relying on convention.

### Verification ideas

DMA and caches add another agent to the memory system. CPU visibility and device visibility must be treated separately when the platform is not fully coherent. The specific question for **Memory leaks** in **59 Allocators** is to make this rule explicit in the code and documentation rather than relying on convention.

### Embedded scenario

Low-memory recovery is itself a resource problem. A system should know what it can still do safely after an allocation failure. The specific question for **Memory leaks** in **59 Allocators** is to make this rule explicit in the code and documentation rather than relying on convention.
