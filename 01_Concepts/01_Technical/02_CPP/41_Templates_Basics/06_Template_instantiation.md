# 41.06 Template instantiation

## Core idea

**Template instantiation** is a concrete part of **Templates and Generic Programming**. Templates parameterize types, values or policies at compile time. The design must make requirements clear and control instantiation, diagnostics and code-generation costs.
...
Templates are compile-time programs. A parameter is useful when it represents a genuine dimension of variation in type, value or policy. The specific question for **Template instantiation** in **41 Templates Basics** is to make this rule explicit in the code and documentation rather than relying on convention.

### Mechanism detail

Instantiation creates concrete code. Many combinations can increase compile time and binary size, even if every individual specialization is small. The specific question for **Template instantiation** in **41 Templates Basics** is to make this rule explicit in the code and documentation rather than relying on convention.
...
Non-type parameters are valuable for fixed capacities and dimensions because the configuration becomes part of the type. The specific question for **Template instantiation** in **41 Templates Basics** is to make this rule explicit in the code and documentation rather than relying on convention.
...
Concepts and requires expressions improve the boundary by stating the operations or properties needed from a template argument. The specific question for **Template instantiation** in **41 Templates Basics** is to make this rule explicit in the code and documentation rather than relying on convention.
