# 35.07 Increment and decrement

## Core idea

**Increment and decrement** is a concrete part of **Operator Overloading**. Expressions are typed computations with precedence, conversions and side effects. Operator behavior must be understood semantically, not just by visual appearance.
...
Boundary cases should be tested early because they expose assumptions that the simple example hides. The specific question for **Increment and decrement** in **35 Operator Overloading** is to make this rule explicit in the code and documentation rather than relying on convention.

### Failure analysis

Portability requires distinguishing standard guarantees from implementation-defined and unspecified behavior. The specific question for **Increment and decrement** in **35 Operator Overloading** is to make this rule explicit in the code and documentation rather than relying on convention.

### Portability note

Use the feature in a deliberately small example before combining it with templates, inheritance, concurrency or hardware-facing code. The specific question for **Increment and decrement** in **35 Operator Overloading** is to make this rule explicit in the code and documentation rather than relying on convention.
...
Document the reason for a non-obvious construct. Future maintainers need the invariant or requirement, not merely the syntax. The specific question for **Increment and decrement** in **35 Operator Overloading** is to make this rule explicit in the code and documentation rather than relying on convention.

### Embedded scenario

Use tests to encode the behavior the team relies on. If a property matters, make it executable evidence. The specific question for **Increment and decrement** in **35 Operator Overloading** is to make this rule explicit in the code and documentation rather than relying on convention.

### Expert review

At Staff level, ask whether the feature reduces system complexity overall or merely relocates it into a harder-to-understand form. The specific question for **Increment and decrement** in **35 Operator Overloading** is to make this rule explicit in the code and documentation rather than relying on convention.
