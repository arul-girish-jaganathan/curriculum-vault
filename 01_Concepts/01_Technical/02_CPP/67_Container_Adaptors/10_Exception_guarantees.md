# 67.10 Exception guarantees

## Core idea

**Exception guarantees** is a concrete part of **stack, queue and priority_queue**. Containers define storage and access policy. Size, capacity, locality, allocation and invalidation rules are as important as the member-function names.
...
Cleanup should happen regardless of where failure occurs. RAII is the standard C++ mechanism for making release behavior structural. The specific question for **Exception guarantees** in **67 Container Adaptors** is to make this rule explicit in the code and documentation rather than relying on convention.
...
Assertions are best for invariants and programmer assumptions that indicate a defect, not for expected runtime input validation. The specific question for **Exception guarantees** in **67 Container Adaptors** is to make this rule explicit in the code and documentation rather than relying on convention.
...
Fault containment reduces the blast radius. A malformed message should not necessarily be able to corrupt unrelated subsystem state. The specific question for **Exception guarantees** in **67 Container Adaptors** is to make this rule explicit in the code and documentation rather than relying on convention.
