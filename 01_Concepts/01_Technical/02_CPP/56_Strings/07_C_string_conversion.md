# 56.07 C-string conversion

## Core idea

**C-string conversion** is a concrete part of **std::string and C Strings**. Text handling combines data representation, ownership, termination/length, encoding and lifetime. These properties must not be conflated.
...
String handling combines data, length, ownership and encoding. A null-terminated string is not interchangeable with an arbitrary byte buffer. The specific question for **C-string conversion** in **56 Strings** is to make this rule explicit in the code and documentation rather than relying on convention.

### Mechanism detail

std::string owns its character storage. A string_view does not; the view is only valid while the underlying character sequence remains alive and stable. The specific question for **C-string conversion** in **56 Strings** is to make this rule explicit in the code and documentation rather than relying on convention.

### Design implication

Capacity and size differ. Growing a string may allocate and may invalidate pointers, references or iterators into its characters. The specific question for **C-string conversion** in **56 Strings** is to make this rule explicit in the code and documentation rather than relying on convention.

### Failure analysis

Character indexing is not automatically Unicode character indexing. Encoding and code-point interpretation belong to the higher-level text contract. The specific question for **C-string conversion** in **56 Strings** is to make this rule explicit in the code and documentation rather than relying on convention.

### Portability note

C-style APIs often infer length from a terminator, while modern interfaces can pass an explicit range. Choosing the latter can make bounds visible. The specific question for **C-string conversion** in **56 Strings** is to make this rule explicit in the code and documentation rather than relying on convention.

### Resource analysis

A view over temporary storage is a classic dangling-lifetime defect. The shorter the source lifetime, the more dangerous a non-owning view becomes. The specific question for **C-string conversion** in **56 Strings** is to make this rule explicit in the code and documentation rather than relying on convention.

### Debugging evidence

Formatting and parsing can be disproportionately expensive on embedded targets. Binary or fixed-format representations may provide more predictable resource use. The specific question for **C-string conversion** in **56 Strings** is to make this rule explicit in the code and documentation rather than relying on convention.

### Verification ideas

String comparison cost scales with input length and may be sensitive to encoding and collation rules. The specific question for **C-string conversion** in **56 Strings** is to make this rule explicit in the code and documentation rather than relying on convention.

### Embedded scenario

Conversions between owned strings, raw character pointers and views need explicit lifetime and mutability reasoning. The specific question for **C-string conversion** in **56 Strings** is to make this rule explicit in the code and documentation rather than relying on convention.

### Expert review

Tests should include empty strings, embedded nulls where the API permits bytes, long inputs, encoding boundaries and allocation-failure scenarios. The specific question for **C-string conversion** in **56 Strings** is to make this rule explicit in the code and documentation rather than relying on convention.
