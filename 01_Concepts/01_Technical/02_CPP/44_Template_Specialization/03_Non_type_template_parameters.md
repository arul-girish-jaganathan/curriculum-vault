# 44.03 Non-type template parameters

## Core idea

**Non-type template parameters** is a concrete part of **Template Specialization**. Templates parameterize types, values or policies at compile time. The design must make requirements clear and control instantiation, diagnostics and code-generation costs.
...
Templates are compile-time programs. A parameter is useful when it represents a genuine dimension of variation in type, value or policy. The specific question for **Non-type template parameters** in **44 Template Specialization** is to make this rule explicit in the code and documentation rather than relying on convention.

### Mechanism detail

Instantiation creates concrete code. Many combinations can increase compile time and binary size, even if every individual specialization is small. The specific question for **Non-type template parameters** in **44 Template Specialization** is to make this rule explicit in the code and documentation rather than relying on convention.

### Design implication

Template argument deduction follows rules involving references, cv-qualification, arrays and function types. Never infer the deduced type from source appearance alone. The specific question for **Non-type template parameters** in **44 Template Specialization** is to make this rule explicit in the code and documentation rather than relying on convention.

### Failure analysis

Template definitions generally need to be visible where they are instantiated, which affects header organization, modules and build dependencies. The specific question for **Non-type template parameters** in **44 Template Specialization** is to make this rule explicit in the code and documentation rather than relying on convention.

### Portability note

Non-type parameters are valuable for fixed capacities and dimensions because the configuration becomes part of the type. The specific question for **Non-type template parameters** in **44 Template Specialization** is to make this rule explicit in the code and documentation rather than relying on convention.

### Resource analysis

Specialization should represent a real semantic or implementation difference. Using specialization as a complicated substitute for ordinary overloads can make maintenance difficult. The specific question for **Non-type template parameters** in **44 Template Specialization** is to make this rule explicit in the code and documentation rather than relying on convention.

### Debugging evidence

Concepts and requires expressions improve the boundary by stating the operations or properties needed from a template argument. The specific question for **Non-type template parameters** in **44 Template Specialization** is to make this rule explicit in the code and documentation rather than relying on convention.

### Verification ideas

Generic code should be tested with multiple representative types, including a type that almost satisfies the requirements and a type that intentionally fails them. The specific question for **Non-type template parameters** in **44 Template Specialization** is to make this rule explicit in the code and documentation rather than relying on convention.

### Embedded scenario

Embedded template code can produce excellent static configuration, but the number of generated combinations must be included in the binary-size budget. The specific question for **Non-type template parameters** in **44 Template Specialization** is to make this rule explicit in the code and documentation rather than relying on convention.

### Expert review

The best generic interface is often the smallest one that captures the real variation while remaining easy to read in compiler diagnostics. The specific question for **Non-type template parameters** in **44 Template Specialization** is to make this rule explicit in the code and documentation rather than relying on convention.
