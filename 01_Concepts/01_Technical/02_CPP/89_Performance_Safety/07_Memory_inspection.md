# 89.07 Memory inspection

## Core idea

**Memory inspection** is a concrete part of **Performance, Safety and Verification**. Debugging is evidence-driven root-cause analysis. Diagnostics, debugger state, instrumentation and tests each answer different questions.
...
Ownership must be unique, shared or borrowed by design. Ambiguous ownership is a common root cause of leaks and double-destruction defects. The specific question for **Memory inspection** in **89 Performance Safety** is to make this rule explicit in the code and documentation rather than relying on convention.
...
Alignment is a correctness requirement, not just a micro-optimization. Packed or manually constructed objects can violate target access requirements. The specific question for **Memory inspection** in **89 Performance Safety** is to make this rule explicit in the code and documentation rather than relying on convention.
...
Low-memory recovery is itself a resource problem. A system should know what it can still do safely after an allocation failure. The specific question for **Memory inspection** in **89 Performance Safety** is to make this rule explicit in the code and documentation rather than relying on convention.
